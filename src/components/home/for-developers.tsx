// @ts-nocheck
/* eslint-disable */
/* This file was generated by Unframer for Framer project ca611bf9ae534859 "SignatureAPI", do not edit manually */
"use client";

// virtual:for-developers
/**
 * @typedef Locale
 * string
 */

/**
 * @typedef {{
 * children?: React.ReactNode
 * locale?: Locale
 * style?: React.CSSProperties
 * className?: string
 * id?: string
 * ref?: any
 * width?: any
 * height?: any
 * layoutId?: string
 * variant?: 'Desktop' | 'Tablet' | 'Phone' // Variant
}} Props

 */
import { Fragment as Fragment4 } from "react";
import { ContextProviders } from "unframer";

// /:https://framer.com/m/for-developers-j22B.js
import { jsx as _jsx4, jsxs as _jsxs3 } from "react/jsx-runtime";
import {
	addFonts as addFonts3,
	addPropertyControls as addPropertyControls3,
	ComponentViewportProvider,
	ControlType as ControlType3,
	cx as cx4,
	getFonts,
	getFontsFromSharedStyle as getFontsFromSharedStyle3,
	getLoadingLazyAtYPosition,
	Image,
	Link as Link2,
	RichText as RichText3,
	SmartComponentScopedContainer,
	useComponentViewport as useComponentViewport3,
	useLocaleInfo as useLocaleInfo3,
	useVariantState as useVariantState3,
	withCSS as withCSS4,
} from "unframer";
import {
	LayoutGroup as LayoutGroup3,
	motion as motion4,
	MotionConfigContext as MotionConfigContext3,
} from "unframer";
import * as React4 from "react";
import { useRef as useRef3 } from "react";

// /:https://framerusercontent.com/modules/FqHwJw9acME2iC9hjK6N/UYPn0DXnIv8OK25aaMXH/BFWLS0wJE.js
import { fontStore } from "unframer";
fontStore.loadFonts(["CUSTOM;SignatureAPI Title Medium"]);
var fonts = [
	{
		explicitInter: true,
		fonts: [
			{
				family: "SignatureAPI Title Medium",
				source: "custom",
				url: "https://framerusercontent.com/assets/AJ18YpHHbCJeeiZ1ProTLuLHhU.woff2",
			},
		],
	},
];
var css = [
	'.framer-OaF3s .framer-styles-preset-clpq28:not(.rich-text-wrapper), .framer-OaF3s .framer-styles-preset-clpq28.rich-text-wrapper h1 { --framer-font-family: "SignatureAPI Title Medium", "SignatureAPI Title Medium Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 48px; --framer-font-style: normal; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-letter-spacing: 0em; --framer-line-height: 52px; --framer-paragraph-spacing: 0px; --framer-text-alignment: center; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; }',
	'@media (max-width: 1199px) and (min-width: 800px) { .framer-OaF3s .framer-styles-preset-clpq28:not(.rich-text-wrapper), .framer-OaF3s .framer-styles-preset-clpq28.rich-text-wrapper h1 { --framer-font-family: "SignatureAPI Title Medium", "SignatureAPI Title Medium Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 42px; --framer-font-style: normal; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-letter-spacing: 0em; --framer-line-height: 48px; --framer-paragraph-spacing: 0px; --framer-text-alignment: center; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
	'@media (max-width: 799px) and (min-width: 0px) { .framer-OaF3s .framer-styles-preset-clpq28:not(.rich-text-wrapper), .framer-OaF3s .framer-styles-preset-clpq28.rich-text-wrapper h1 { --framer-font-family: "SignatureAPI Title Medium", "SignatureAPI Title Medium Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 32px; --framer-font-style: normal; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-letter-spacing: 0em; --framer-line-height: 40px; --framer-paragraph-spacing: 0px; --framer-text-alignment: center; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
];
var className = "framer-OaF3s";

// /:https://framerusercontent.com/modules/rFqRsRnyaNaZlLQbSh5i/PIUlA6NT8IBrL2dhcXre/GuhMGcyuK.js
import { fontStore as fontStore2 } from "unframer";
fontStore2.loadFonts([
	"Inter-Medium",
	"Inter-Bold",
	"Inter-BoldItalic",
	"Inter-MediumItalic",
]);
var fonts2 = [
	{
		explicitInter: true,
		fonts: [
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/5A3Ce6C9YYmCjpQx9M4inSaKU.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/Qx95Xyt0Ka3SGhinnbXIGpEIyP4.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/6mJuEAguuIuMog10gGvH5d3cl8.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/xYYWaj7wCU5zSQH0eXvSaS19wo.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/otTaNuNpVK4RbdlT7zDDdKvQBA.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/d3tHnaQIAeqiE5hGcRw4mmgWYU.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/DolVirEGb34pEXEp8t8FQBSK4.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/DpPBYI0sL4fYLgAkX8KXOPVt7c.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/4RAEQdEOrcnDkhHiiCbJOw92Lk.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/1K3W8DizY3v4emK8Mb08YHxTbs.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/tUSCtfYVM1I1IchuyCwz9gDdQ.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/VgYFWiwsAC5OYxAycRXXvhze58.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/DXD0Q7LSl7HEvDzucnyLnGBHM.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/GIryZETIX4IFypco5pYZONKhJIo.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/H89BbHkbHDzlxZzxi8uPzTsp90.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/u6gJwDuwB143kpNK1T1MDKDWkMc.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/43sJ6MfOPh1LCJt46OvyDuSbA6o.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/wccHG0r4gBDAIRhfHiOlq6oEkqw.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/WZ367JPwf9bRW6LdTHN8rXgSjw.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/QxmhnWTzLtyjIiZcfaLIJ8EFBXU.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/2A4Xx7CngadFGlVV4xrO06OBHY.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/khkJkwSL66WFg8SX6Wa726c.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/0E7IMbDzcGABpBwwqNEt60wU0w.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/NTJ0nQgIF0gcDelS14zQ9NR9Q.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/QrcNhgEPfRl0LS8qz5Ln8olanl8.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/JEXmejW8mXOYMtt0hyRg811kHac.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/ksvR4VsLksjpSwnC2fPgHRNMw.woff2",
				weight: "500",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/uy9s0iWuxiNnVt8EpTI3gzohpwo.woff2",
				weight: "500",
			},
		],
	},
];
var css2 = [
	'.framer-V3TRN .framer-styles-preset-12cf9y3:not(.rich-text-wrapper), .framer-V3TRN .framer-styles-preset-12cf9y3.rich-text-wrapper p { --framer-font-family: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 14px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: italic; --framer-font-style-italic: italic; --framer-font-variation-axes: normal; --framer-font-weight: 500; --framer-font-weight-bold: 700; --framer-font-weight-bold-italic: 700; --framer-font-weight-italic: 500; --framer-letter-spacing: 0em; --framer-line-height: 20px; --framer-paragraph-spacing: 20px; --framer-text-alignment: start; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; }',
];
var className2 = "framer-V3TRN";

// /:https://framerusercontent.com/modules/UItcSeM7AdIGPKOFdoHe/sZkSK7OL3ODbIUs3T7j0/olW1GQM24.js
import { fontStore as fontStore3 } from "unframer";
fontStore3.loadFonts([
	"Inter",
	"Inter-VariableVF=Im9wc3oiIDE0LCAid2dodCIgMzUw",
	"Inter-VariableVF=Im9wc3oiIDE0LCAid2dodCIgMzUw",
	"Inter-VariableVF=Im9wc3oiIDE0LCAid2dodCIgMzUw",
]);
var fonts3 = [
	{
		explicitInter: true,
		fonts: [
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/5vvr9Vy74if2I6bQbJvbw7SY1pQ.woff2",
				weight: "400",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/EOr0mi4hNtlgWNn9if640EZzXCo.woff2",
				weight: "400",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/Y9k9QrlZAqio88Klkmbd8VoMQc.woff2",
				weight: "400",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/OYrD2tBIBPvoJXiIHnLoOXnY9M.woff2",
				weight: "400",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/JeYwfuaPfZHQhEG8U5gtPDZ7WQ.woff2",
				weight: "400",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/vQyevYAyHtARFwPqUzQGpnDs.woff2",
				weight: "400",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/b6Y37FthZeALduNqHicBT6FutY.woff2",
				weight: "400",
			},
		],
	},
];
var css3 = [
	'.framer-Dw27X .framer-styles-preset-e44ul3:not(.rich-text-wrapper), .framer-Dw27X .framer-styles-preset-e44ul3.rich-text-wrapper p { --framer-font-family: "Inter", sans-serif; --framer-font-family-bold: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 18px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: normal; --framer-font-style-italic: normal; --framer-font-variation-axes: normal; --framer-font-variation-axes-bold: "opsz" 14, "wght" 350; --framer-font-variation-axes-bold-italic: "opsz" 14, "wght" 350; --framer-font-variation-axes-italic: "opsz" 14, "wght" 350; --framer-font-weight: 400; --framer-font-weight-bold: 400; --framer-font-weight-bold-italic: 400; --framer-font-weight-italic: 400; --framer-letter-spacing: -0.01em; --framer-line-height: 28px; --framer-paragraph-spacing: 20px; --framer-text-alignment: center; --framer-text-color: var(--token-731c96e3-6087-4671-b6ea-f45a74a8d152, #3f3f46); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; }',
	'@media (max-width: 1199px) and (min-width: 800px) { .framer-Dw27X .framer-styles-preset-e44ul3:not(.rich-text-wrapper), .framer-Dw27X .framer-styles-preset-e44ul3.rich-text-wrapper p { --framer-font-family: "Inter", sans-serif; --framer-font-family-bold: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 16px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: normal; --framer-font-style-italic: normal; --framer-font-variation-axes: normal; --framer-font-variation-axes-bold: "opsz" 14, "wght" 350; --framer-font-variation-axes-bold-italic: "opsz" 14, "wght" 350; --framer-font-variation-axes-italic: "opsz" 14, "wght" 350; --framer-font-weight: 400; --framer-font-weight-bold: 400; --framer-font-weight-bold-italic: 400; --framer-font-weight-italic: 400; --framer-letter-spacing: -0.01em; --framer-line-height: 24px; --framer-paragraph-spacing: 20px; --framer-text-alignment: center; --framer-text-color: var(--token-731c96e3-6087-4671-b6ea-f45a74a8d152, #3f3f46); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
	'@media (max-width: 799px) and (min-width: 0px) { .framer-Dw27X .framer-styles-preset-e44ul3:not(.rich-text-wrapper), .framer-Dw27X .framer-styles-preset-e44ul3.rich-text-wrapper p { --framer-font-family: "Inter", sans-serif; --framer-font-family-bold: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter Variable", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 16px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: normal; --framer-font-style-italic: normal; --framer-font-variation-axes: normal; --framer-font-variation-axes-bold: "opsz" 14, "wght" 350; --framer-font-variation-axes-bold-italic: "opsz" 14, "wght" 350; --framer-font-variation-axes-italic: "opsz" 14, "wght" 350; --framer-font-weight: 400; --framer-font-weight-bold: 400; --framer-font-weight-bold-italic: 400; --framer-font-weight-italic: 400; --framer-letter-spacing: -0.01em; --framer-line-height: 24px; --framer-paragraph-spacing: 20px; --framer-text-alignment: center; --framer-text-color: var(--token-731c96e3-6087-4671-b6ea-f45a74a8d152, #3f3f46); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
];
var className3 = "framer-Dw27X";

// /:https://framerusercontent.com/modules/Lo0bitSFLbUOKJqs1zOC/V69q3N7OZpEybHwUC35O/vpUju7Kmv.js
import { fontStore as fontStore4 } from "unframer";
fontStore4.loadFonts(["CUSTOM;SignatureAPI Title Medium"]);
var fonts4 = [
	{
		explicitInter: true,
		fonts: [
			{
				family: "SignatureAPI Title Medium",
				source: "custom",
				url: "https://framerusercontent.com/assets/AJ18YpHHbCJeeiZ1ProTLuLHhU.woff2",
			},
		],
	},
];
var css4 = [
	'.framer-zILgU .framer-styles-preset-1v2y8dz:not(.rich-text-wrapper), .framer-zILgU .framer-styles-preset-1v2y8dz.rich-text-wrapper h4 { --framer-font-family: "SignatureAPI Title Medium", "SignatureAPI Title Medium Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 20px; --framer-font-style: normal; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-letter-spacing: 0.02em; --framer-line-height: 28px; --framer-paragraph-spacing: 0px; --framer-text-alignment: start; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; }',
	'@media (max-width: 1199px) and (min-width: 800px) { .framer-zILgU .framer-styles-preset-1v2y8dz:not(.rich-text-wrapper), .framer-zILgU .framer-styles-preset-1v2y8dz.rich-text-wrapper h4 { --framer-font-family: "SignatureAPI Title Medium", "SignatureAPI Title Medium Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 18px; --framer-font-style: normal; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-letter-spacing: 0.02em; --framer-line-height: 26px; --framer-paragraph-spacing: 0px; --framer-text-alignment: start; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
	'@media (max-width: 799px) and (min-width: 0px) { .framer-zILgU .framer-styles-preset-1v2y8dz:not(.rich-text-wrapper), .framer-zILgU .framer-styles-preset-1v2y8dz.rich-text-wrapper h4 { --framer-font-family: "SignatureAPI Title Medium", "SignatureAPI Title Medium Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 16px; --framer-font-style: normal; --framer-font-variation-axes: normal; --framer-font-weight: 400; --framer-letter-spacing: 0.02em; --framer-line-height: 24px; --framer-paragraph-spacing: 0px; --framer-text-alignment: start; --framer-text-color: var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, #18181b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; } }',
];
var className4 = "framer-zILgU";

// /:https://framerusercontent.com/modules/vCL82hVYyDSatg2EZ2LT/7C50hF8TbgVaoC20yuPG/FHPqRCV9w.js
import { jsx as _jsx } from "react/jsx-runtime";
import { cx, motion, withCSS } from "unframer";
import * as React from "react";
import { forwardRef as forwardRef2 } from "react";
var mask = "var(--framer-icon-mask)";
var SVG = /* @__PURE__ */ forwardRef2((props, ref) => {
	const { animated, layoutId, children, ...rest } = props;
	return animated ? (
		<motion.img {...rest} layoutId={layoutId} ref={ref} />
	) : (
		<img {...rest} ref={ref} />
	);
});
var getProps = ({ height, id, width, ...props }) => {
	return {
		...props,
	};
};
var Component = /* @__PURE__ */ React.forwardRef(function (props, ref) {
	const {
		style,
		className: className7,
		layoutId,
		variant,
		...restProps
	} = getProps(props);
	return (
		<SVG
			{...restProps}
			alt={""}
			className={cx("framer-QYSrD", className7)}
			layoutId={layoutId}
			ref={ref}
			src={
				'data:image/svg+xml,<svg display="block" role="presentation" viewBox="0 0 1360 840" xmlns="http://www.w3.org/2000/svg"><g d="M 0 0 L 1360 0 L 1360 840 L 0 840 Z M 804.365 0 L 4.85 840 L 3.393 840 L 802.908 0 Z M 812.607 0 L 13.092 840 L 11.635 840 L 811.15 0 Z M 820.85 0 L 21.335 840 L 19.878 840 L 819.393 0 Z M 829.092 0 L 29.577 840 L 28.12 840 L 827.635 0 Z M 837.334 0 L 37.819 840 L 36.362 840 L 835.878 0 Z M 845.577 0 L 46.062 840 L 44.605 840 L 844.12 0 Z M 853.819 0 L 54.304 840 L 52.847 840 L 852.362 0 Z M 862.062 0 L 62.547 840 L 61.09 840 L 860.605 0 Z M 870.304 0 L 70.789 840 L 69.332 840 L 868.847 0 Z M 878.547 0 L 79.032 840 L 77.575 840 L 877.09 0 Z M 886.789 0 L 87.274 840 L 85.817 840 L 885.332 0 Z M 895.031 0 L 95.516 840 L 94.06 840 L 893.575 0 Z M 903.274 0 L 103.759 840 L 102.302 840 L 901.817 0 Z M 911.516 0 L 112.001 840 L 110.544 840 L 910.059 0 Z M 919.759 0 L 120.244 840 L 118.787 840 L 918.302 0 Z M 928.001 0 L 128.486 840 L 127.029 840 L 926.544 0 Z M 936.244 0 L 136.728 840 L 135.272 840 L 934.787 0 Z M 944.486 0 L 144.971 840 L 143.514 840 L 943.029 0 Z M 952.728 0 L 153.213 840 L 151.756 840 L 951.272 0 Z M 960.971 0 L 161.456 840 L 159.999 840 L 959.514 0 Z M 969.213 0 L 169.698 840 L 168.241 840 L 967.756 0 Z M 977.456 0 L 177.941 840 L 176.484 840 L 975.999 0 Z M 985.698 0 L 186.183 840 L 184.726 840 L 984.241 0 Z M 993.941 0 L 194.425 840 L 192.969 840 L 992.484 0 Z M 1002.18 0 L 202.668 840 L 201.211 840 L 1000.73 0 Z M 1010.43 0 L 210.91 840 L 209.453 840 L 1008.97 0 Z M 1018.67 0 L 219.153 840 L 217.696 840 L 1017.21 0 Z M 1026.91 0 L 227.395 840 L 225.938 840 L 1025.45 0 Z M 1035.15 0 L 235.638 840 L 234.181 840 L 1033.7 0 Z M 1043.4 0 L 243.88 840 L 242.423 840 L 1041.94 0 Z M 1051.64 0 L 252.122 840 L 250.665 840 L 1050.18 0 Z M 1059.88 0 L 260.365 840 L 258.908 840 L 1058.42 0 Z M 1068.12 0 L 268.607 840 L 267.15 840 L 1066.67 0 Z M 1076.36 0 L 276.85 840 L 275.393 840 L 1074.91 0 Z M 1084.61 0 L 285.092 840 L 283.635 840 L 1083.15 0 Z M 1092.85 0 L 293.335 840 L 291.878 840 L 1091.39 0 Z M 1101.09 0 L 301.577 840 L 300.12 840 L 1099.64 0 Z M 1109.33 0 L 309.819 840 L 308.362 840 L 1107.88 0 Z M 1117.58 0 L 318.062 840 L 316.605 840 L 1116.12 0 Z M 1125.82 0 L 326.304 840 L 324.847 840 L 1124.36 0 Z M 1134.06 0 L 334.547 840 L 333.09 840 L 1132.6 0 Z M 1142.3 0 L 342.789 840 L 341.332 840 L 1140.85 0 Z M 1150.55 0 L 351.031 840 L 349.575 840 L 1149.09 0 Z M 1158.79 0 L 359.274 840 L 357.817 840 L 1157.33 0 Z M 1167.03 0 L 367.516 840 L 366.059 840 L 1165.57 0 Z M 1175.27 0 L 375.759 840 L 374.302 840 L 1173.82 0 Z M 1183.52 0 L 384.001 840 L 382.544 840 L 1182.06 0 Z M 1191.76 0 L 392.244 840 L 390.787 840 L 1190.3 0 Z M 1200 0 L 400.486 840 L 399.029 840 L 1198.54 0 Z M 1208.24 0 L 408.728 840 L 407.272 840 L 1206.79 0 Z M 1216.49 0 L 416.971 840 L 415.514 840 L 1215.03 0 Z M 1224.73 0 L 425.213 840 L 423.756 840 L 1223.27 0 Z M 1232.97 0 L 433.456 840 L 431.999 840 L 1231.51 0 Z M 1241.21 0 L 441.698 840 L 440.241 840 L 1239.76 0 Z M 1249.46 0 L 449.941 840 L 448.484 840 L 1248 0 Z M 1257.7 0 L 458.183 840 L 456.726 840 L 1256.24 0 Z M 1265.94 0 L 466.425 840 L 464.969 840 L 1264.48 0 Z M 1274.18 0 L 474.668 840 L 473.211 840 L 1272.73 0 Z M 1282.43 0 L 482.91 840 L 481.453 840 L 1280.97 0 Z M 1290.67 0 L 491.153 840 L 489.696 840 L 1289.21 0 Z M 1298.91 0 L 499.395 840 L 497.938 840 L 1297.45 0 Z M 1307.15 0 L 507.638 840 L 506.181 840 L 1305.7 0 Z M 1315.4 0 L 515.88 840 L 514.423 840 L 1313.94 0 Z M 1323.64 0 L 524.122 840 L 522.665 840 L 1322.18 0 Z M 1331.88 0 L 532.365 840 L 530.908 840 L 1330.42 0 Z M 1340.12 0 L 540.607 840 L 539.15 840 L 1338.67 0 Z M 1348.36 0 L 548.85 840 L 547.393 840 L 1346.91 0 Z M 1356.61 0 L 557.092 840 L 555.635 840 L 1355.15 0 Z M 1360 5.095 L 565.335 840 L 563.878 840 L 1360 3.565 Z M 1360 13.755 L 573.577 840 L 572.12 840 L 1360 12.224 Z M 1360 22.415 L 581.819 840 L 580.362 840 L 1360 20.884 L 1360 22.414 Z M 1360 31.075 L 590.062 840 L 588.605 840 L 1360 29.544 L 1360 31.074 Z M 1360 39.735 L 598.304 840 L 596.847 840 L 1360 38.204 L 1360 39.734 Z M 1360 48.395 L 606.547 840 L 605.09 840 L 1360 46.864 L 1360 48.394 Z M 1360 57.054 L 614.789 840 L 613.332 840 L 1360 55.523 Z M 1360 65.714 L 623.031 840 L 621.575 840 L 1360 64.183 L 1360 65.713 Z M 1360 74.374 L 631.274 840 L 629.817 840 L 1360 72.843 L 1360 74.373 Z M 1360 83.034 L 639.516 840 L 638.059 840 L 1360 81.503 L 1360 83.033 Z M 1360 91.694 L 647.759 840 L 646.302 840 L 1360 90.163 L 1360 91.693 Z M 1360 100.353 L 656.001 840 L 654.544 840 L 1360 98.822 Z M 1360 109.013 L 664.244 840 L 662.787 840 L 1360 107.482 Z M 1360 117.673 L 672.486 840 L 671.029 840 L 1360 116.142 Z M 1360 126.332 L 680.728 840 L 679.272 840 L 1360 124.802 Z M 1360 134.992 L 688.971 840 L 687.514 840 L 1360 133.461 Z M 1360 143.652 L 697.213 840 L 695.756 840 L 1360 142.121 Z M 1360 152.312 L 705.456 840 L 703.999 840 L 1360 150.781 Z M 1360 160.972 L 713.698 840 L 712.241 840 L 1360 159.441 Z M 1360 169.631 L 721.941 840 L 720.484 840 L 1360 168.101 Z M 1360 178.291 L 730.183 840 L 728.726 840 L 1360 176.76 Z M 1360 186.951 L 738.425 840 L 736.968 840 L 1360 185.42 Z M 1360 195.611 L 746.668 840 L 745.211 840 L 1360 194.08 Z M 1360 204.27 L 754.91 840 L 753.453 840 L 1360 202.74 Z M 1360 212.93 L 763.153 840 L 761.696 840 L 1360 211.4 Z M 1360 221.59 L 771.395 840 L 769.938 840 L 1360 220.059 Z M 1360 230.25 L 779.638 840 L 778.181 840 L 1360 228.719 Z M 1360 238.91 L 787.88 840 L 786.423 840 L 1360 237.379 Z M 1360 247.569 L 796.122 840 L 794.665 840 L 1360 246.039 Z M 1360 256.229 L 804.365 840 L 802.908 840 L 1360 254.699 Z M 1360 264.889 L 812.607 840 L 811.15 840 L 1360 263.358 Z M 1360 273.549 L 820.85 840 L 819.393 840 L 1360 272.018 Z M 1360 282.209 L 829.092 840 L 827.635 840 L 1360 280.678 Z M 1360 290.868 L 837.334 840 L 835.878 840 L 1360 289.338 Z M 1360 299.528 L 845.577 840 L 844.12 840 L 1360 297.998 Z M 1360 308.188 L 853.819 840 L 852.362 840 L 1360 306.657 Z M 1360 316.848 L 862.062 840 L 860.605 840 L 1360 315.317 Z M 1360 325.508 L 870.304 840 L 868.847 840 L 1360 323.977 Z M 1360 334.167 L 878.547 840 L 877.09 840 L 1360 332.637 Z M 1360 342.827 L 886.789 840 L 885.332 840 L 1360 341.297 Z M 1360 351.487 L 895.031 840 L 893.575 840 L 1360 349.956 Z M 1360 360.147 L 903.274 840 L 901.817 840 L 1360 358.616 Z M 1360 368.807 L 911.516 840 L 910.059 840 L 1360 367.276 Z M 1360 377.466 L 919.759 840 L 918.302 840 L 1360 375.936 Z M 1360 386.126 L 928.001 840 L 926.544 840 L 1360 384.595 Z M 1360 394.786 L 936.244 840 L 934.787 840 L 1360 393.255 Z M 1360 403.446 L 944.486 840 L 943.029 840 L 1360 401.915 Z M 1360 412.106 L 952.728 840 L 951.272 840 L 1360 410.575 Z M 1360 420.765 L 960.971 840 L 959.514 840 L 1360 419.235 Z M 1360 429.425 L 969.213 840 L 967.756 840 L 1360 427.894 Z M 1360 438.085 L 977.456 840 L 975.999 840 L 1360 436.554 Z M 1360 446.745 L 985.698 840 L 984.241 840 L 1360 445.214 Z M 1360 455.405 L 993.941 840 L 992.484 840 L 1360 453.874 Z M 1360 464.064 L 1002.18 840 L 1000.73 840 L 1360 462.534 Z M 1360 472.724 L 1010.43 840 L 1008.97 840 L 1360 471.193 Z M 1360 481.384 L 1018.67 840 L 1017.21 840 L 1360 479.853 Z M 1360 490.044 L 1026.91 840 L 1025.45 840 L 1360 488.513 Z M 1360 498.703 L 1035.15 840 L 1033.7 840 L 1360 497.173 Z M 1360 507.363 L 1043.4 840 L 1041.94 840 L 1360 505.833 Z M 1360 516.023 L 1051.64 840 L 1050.18 840 L 1360 514.492 Z M 1360 524.683 L 1059.88 840 L 1058.42 840 L 1360 523.152 Z M 1360 533.343 L 1068.12 840 L 1066.67 840 L 1360 531.812 Z M 1360 542.002 L 1076.36 840 L 1074.91 840 L 1360 540.472 Z M 1360 550.662 L 1084.61 840 L 1083.15 840 L 1360 549.132 Z M 1360 559.322 L 1092.85 840 L 1091.39 840 L 1360 557.791 Z M 1360 567.982 L 1101.09 840 L 1099.64 840 L 1360 566.451 Z M 1360 576.642 L 1109.33 840 L 1107.88 840 L 1360 575.111 Z M 1360 585.301 L 1117.58 840 L 1116.12 840 L 1360 583.771 Z M 1360 593.961 L 1125.82 840 L 1124.36 840 L 1360 592.431 Z M 1360 602.621 L 1134.06 840 L 1132.6 840 L 1360 601.09 Z M 1360 611.281 L 1142.3 840 L 1140.85 840 L 1360 609.75 Z M 1360 619.941 L 1150.55 840 L 1149.09 840 L 1360 618.41 Z M 1360 628.6 L 1158.79 840 L 1157.33 840 L 1360 627.07 Z M 1360 637.26 L 1167.03 840 L 1165.57 840 L 1360 635.73 Z M 1360 645.92 L 1175.27 840 L 1173.82 840 L 1360 644.389 Z M 1360 654.58 L 1183.52 840 L 1182.06 840 L 1360 653.049 Z M 1360 663.24 L 1191.76 840 L 1190.3 840 L 1360 661.709 Z M 1360 671.899 L 1200 840 L 1198.54 840 L 1360 670.369 Z M 1360 680.559 L 1208.24 840 L 1206.79 840 L 1360 679.028 Z M 1360 689.219 L 1216.49 840 L 1215.03 840 L 1360 687.688 Z M 1360 697.879 L 1224.73 840 L 1223.27 840 L 1360 696.348 Z M 1360 706.539 L 1232.97 840 L 1231.51 840 L 1360 705.008 Z M 1360 715.198 L 1241.21 840 L 1239.76 840 L 1360 713.668 Z M 1360 723.858 L 1249.46 840 L 1248 840 L 1360 722.327 Z M 1360 732.518 L 1257.7 840 L 1256.24 840 L 1360 730.987 Z M 1360 741.178 L 1265.94 840 L 1264.48 840 L 1360 739.647 Z M 1360 749.838 L 1274.18 840 L 1272.73 840 L 1360 748.307 Z M 1360 758.497 L 1282.43 840 L 1280.97 840 L 1360 756.967 Z M 1360 767.157 L 1290.67 840 L 1289.21 840 L 1360 765.626 Z M 1360 775.817 L 1298.91 840 L 1297.45 840 L 1360 774.286 Z M 1360 784.477 L 1307.15 840 L 1305.7 840 L 1360 782.946 Z M 1360 793.136 L 1315.4 840 L 1313.94 840 L 1360 791.606 Z M 1360 801.796 L 1323.64 840 L 1322.18 840 L 1360 800.266 Z M 1360 810.456 L 1331.88 840 L 1330.42 840 L 1360 808.925 Z M 1360 819.116 L 1340.12 840 L 1338.67 840 L 1360 817.585 Z M 1360 827.776 L 1348.36 840 L 1346.91 840 L 1360 826.245 Z M 1360 836.435 L 1356.61 840 L 1355.15 840 L 1360 834.905 Z M 796.122 0 L 0 836.435 L 0 834.905 L 794.665 0 Z M 787.88 0 L 0 827.776 L 0 826.245 L 786.423 0 Z M 779.638 0 L 0 819.116 L 0 817.585 L 778.181 0 Z M 771.395 0 L 0 810.456 L 0 808.925 L 769.938 0 Z M 763.153 0 L 0 801.796 L 0 800.266 L 761.696 0 Z M 754.91 0 L 0 793.136 L 0 791.606 L 753.453 0 Z M 746.668 0 L 0 784.477 L 0 782.946 L 745.211 0 Z M 738.425 0 L 0 775.817 L 0 774.286 L 736.968 0 Z M 730.183 0 L 0 767.157 L 0 765.626 L 728.726 0 Z M 721.941 0 L 0 758.497 L 0 756.967 L 720.484 0 Z M 713.698 0 L 0 749.838 L 0 748.307 L 712.241 0 Z M 705.456 0 L 0 741.178 L 0 739.647 L 703.999 0 Z M 697.213 0 L 0 732.518 L 0 730.987 L 695.756 0 Z M 688.971 0 L 0 723.858 L 0 722.327 L 687.514 0 Z M 680.728 0 L 0 715.198 L 0 713.668 L 679.272 0 Z M 672.486 0 L 0 706.539 L 0 705.008 L 671.029 0 Z M 664.244 0 L 0 697.879 L 0 696.348 L 662.787 0 Z M 656.001 0 L 0 689.219 L 0 687.688 L 654.544 0 Z M 647.759 0 L 0 680.559 L 0 679.028 L 646.302 0 Z M 639.516 0 L 0 671.899 L 0 670.369 L 638.059 0 Z M 631.274 0 L 0 663.24 L 0 661.709 L 629.817 0 Z M 623.031 0 L 0 654.58 L 0 653.049 L 621.575 0 Z M 614.789 0 L 0 645.92 L 0 644.389 L 613.332 0 Z M 606.547 0 L 0 637.26 L 0 635.73 L 605.09 0 Z M 598.304 0 L 0 628.6 L 0 627.07 L 596.847 0 Z M 590.062 0 L 0 619.941 L 0 618.41 L 588.605 0 Z M 581.819 0 L 0 611.281 L 0 609.75 L 580.362 0 Z M 573.577 0 L 0 602.621 L 0 601.09 L 572.12 0 Z M 565.335 0 L 0 593.961 L 0 592.431 L 563.878 0 Z M 557.092 0 L 0 585.301 L 0 583.771 L 555.635 0 Z M 548.85 0 L 0 576.642 L 0 575.111 L 547.393 0 Z M 540.607 0 L 0 567.982 L 0 566.451 L 539.15 0 Z M 532.365 0 L 0 559.322 L 0 557.791 L 530.908 0 Z M 524.122 0 L 0 550.662 L 0 549.132 L 522.665 0 Z M 515.88 0 L 0 542.002 L 0 540.472 L 514.423 0 Z M 507.638 0 L 0 533.343 L 0 531.812 L 506.181 0 Z M 499.395 0 L 0 524.683 L 0 523.152 L 497.938 0 Z M 491.153 0 L 0 516.023 L 0 514.492 L 489.696 0 Z M 482.91 0 L 0 507.363 L 0 505.833 L 481.453 0 Z M 474.668 0 L 0 498.703 L 0 497.173 L 473.211 0 Z M 466.425 0 L 0 490.044 L 0 488.513 L 464.969 0 Z M 458.183 0 L 0 481.384 L 0 479.853 L 456.726 0 Z M 449.941 0 L 0 472.724 L 0 471.193 L 448.484 0 Z M 441.698 0 L 0 464.064 L 0 462.534 L 440.241 0 Z M 433.456 0 L 0 455.405 L 0 453.874 L 431.999 0 Z M 425.213 0 L 0 446.745 L 0 445.214 L 423.756 0 Z M 416.971 0 L 0 438.085 L 0 436.554 L 415.514 0 Z M 408.728 0 L 0 429.425 L 0 427.894 L 407.272 0 Z M 400.486 0 L 0 420.765 L 0 419.235 L 399.029 0 Z M 392.244 0 L 0 412.106 L 0 410.575 L 390.787 0 Z M 384.001 0 L 0 403.446 L 0 401.915 L 382.544 0 Z M 375.759 0 L 0 394.786 L 0 393.255 L 374.302 0 Z M 367.516 0 L 0 386.126 L 0 384.595 L 366.059 0 Z M 359.274 0 L 0 377.466 L 0 375.936 L 357.817 0 Z M 351.031 0 L 0 368.807 L 0 367.276 L 349.575 0 Z M 342.789 0 L 0 360.147 L 0 358.616 L 341.332 0 Z M 334.547 0 L 0 351.487 L 0 349.956 L 333.09 0 Z M 326.304 0 L 0 342.827 L 0 341.297 L 324.847 0 Z M 318.062 0 L 0 334.167 L 0 332.637 L 316.605 0 Z M 309.819 0 L 0 325.508 L 0 323.977 L 308.362 0 Z M 301.577 0 L 0 316.848 L 0 315.317 L 300.12 0 Z M 293.335 0 L 0 308.188 L 0 306.657 L 291.878 0 Z M 285.092 0 L 0 299.528 L 0 297.998 L 283.635 0 Z M 276.85 0 L 0 290.868 L 0 289.338 L 275.393 0 Z M 268.607 0 L 0 282.209 L 0 280.678 L 267.15 0 Z M 260.365 0 L 0 273.549 L 0 272.018 L 258.908 0 Z M 252.122 0 L 0 264.889 L 0 263.358 L 250.665 0 Z M 243.88 0 L 0 256.229 L 0 254.699 L 242.423 0 Z M 235.638 0 L 0 247.569 L 0 246.039 L 234.181 0 Z M 227.395 0 L 0 238.91 L 0 237.379 L 225.938 0 Z M 219.153 0 L 0 230.25 L 0 228.719 L 217.696 0 Z M 210.91 0 L 0 221.59 L 0 220.059 L 209.453 0 Z M 202.668 0 L 0 212.93 L 0 211.4 L 201.211 0 Z M 194.425 0 L 0 204.27 L 0 202.74 L 192.969 0 Z M 186.183 0 L 0 195.611 L 0 194.08 L 184.726 0 Z M 177.941 0 L 0 186.951 L 0 185.42 L 176.484 0 Z M 169.698 0 L 0 178.291 L 0 176.76 L 168.241 0 Z M 161.456 0 L 0 169.631 L 0 168.101 L 159.999 0 Z M 153.213 0 L 0 160.972 L 0 159.441 L 151.756 0 Z M 144.971 0 L 0 152.312 L 0 150.781 L 143.514 0 Z M 136.728 0 L 0 143.652 L 0 142.121 L 135.272 0 Z M 128.486 0 L 0 134.992 L 0 133.461 L 127.029 0 Z M 120.244 0 L 0 126.332 L 0 124.802 L 118.787 0 Z M 112.001 0 L 0 117.673 L 0 116.142 L 110.544 0 Z M 103.759 0 L 0 109.013 L 0 107.482 L 102.302 0 Z M 95.516 0 L 0 100.353 L 0 98.823 L 94.06 0 Z M 87.274 0 L 0 91.693 L 0 90.163 L 85.817 0 Z M 79.03 0 L 0 83.033 L 0 81.503 L 77.575 0 L 79.031 0 Z M 70.79 0 L 0 74.374 L 0 72.843 L 69.332 0 L 70.789 0 Z M 62.548 0 L 0 65.714 L 0 64.184 L 61.09 0 L 62.547 0 Z M 54.305 0 L 0 57.054 L 0 55.524 L 52.847 0 L 54.304 0 Z M 46.063 0 L 0 48.394 L 0 46.864 L 44.605 0 L 46.062 0 Z M 37.82 0 L 0 39.734 L 0 38.204 L 36.362 0 L 37.819 0 Z M 29.578 0 L 0 31.075 L 0 29.544 L 28.12 0 L 29.577 0 Z M 21.335 0 L 0 22.415 L 0 20.885 L 19.878 0 L 21.334 0 Z M 13.093 0 L 0 13.755 L 0 12.225 L 11.635 0 L 13.092 0 Z M 4.85 0 L 0 5.095 L 0 3.565 L 3.393 0 Z" fill="transparent" height="840.0000302734375px" id="z3fKSU5pK" width="1360.0000585937505px"><path d="M 0 0 L 1360 0 L 1360 840 L 0 840 Z" fill="transparent" height="840px" id="yRER2zQRv" width="1360px"/><path d="M 804.365 0 L 4.85 840 L 3.393 840 L 802.908 0 Z M 812.607 0 L 13.092 840 L 11.635 840 L 811.15 0 Z M 820.85 0 L 21.335 840 L 19.878 840 L 819.393 0 Z M 829.092 0 L 29.577 840 L 28.12 840 L 827.635 0 Z M 837.334 0 L 37.819 840 L 36.362 840 L 835.878 0 Z M 845.577 0 L 46.062 840 L 44.605 840 L 844.12 0 Z M 853.819 0 L 54.304 840 L 52.847 840 L 852.362 0 Z M 862.062 0 L 62.547 840 L 61.09 840 L 860.605 0 Z M 870.304 0 L 70.789 840 L 69.332 840 L 868.847 0 Z M 878.547 0 L 79.032 840 L 77.575 840 L 877.09 0 Z M 886.789 0 L 87.274 840 L 85.817 840 L 885.332 0 Z M 895.031 0 L 95.516 840 L 94.06 840 L 893.575 0 Z M 903.274 0 L 103.759 840 L 102.302 840 L 901.817 0 Z M 911.516 0 L 112.001 840 L 110.544 840 L 910.059 0 Z M 919.759 0 L 120.244 840 L 118.787 840 L 918.302 0 Z M 928.001 0 L 128.486 840 L 127.029 840 L 926.544 0 Z M 936.244 0 L 136.728 840 L 135.272 840 L 934.787 0 Z M 944.486 0 L 144.971 840 L 143.514 840 L 943.029 0 Z M 952.728 0 L 153.213 840 L 151.756 840 L 951.272 0 Z M 960.971 0 L 161.456 840 L 159.999 840 L 959.514 0 Z M 969.213 0 L 169.698 840 L 168.241 840 L 967.756 0 Z M 977.456 0 L 177.941 840 L 176.484 840 L 975.999 0 Z M 985.698 0 L 186.183 840 L 184.726 840 L 984.241 0 Z M 993.941 0 L 194.425 840 L 192.969 840 L 992.484 0 Z M 1002.18 0 L 202.668 840 L 201.211 840 L 1000.73 0 Z M 1010.43 0 L 210.91 840 L 209.453 840 L 1008.97 0 Z M 1018.67 0 L 219.153 840 L 217.696 840 L 1017.21 0 Z M 1026.91 0 L 227.395 840 L 225.938 840 L 1025.45 0 Z M 1035.15 0 L 235.638 840 L 234.181 840 L 1033.7 0 Z M 1043.4 0 L 243.88 840 L 242.423 840 L 1041.94 0 Z M 1051.64 0 L 252.122 840 L 250.665 840 L 1050.18 0 Z M 1059.88 0 L 260.365 840 L 258.908 840 L 1058.42 0 Z M 1068.12 0 L 268.607 840 L 267.15 840 L 1066.67 0 Z M 1076.36 0 L 276.85 840 L 275.393 840 L 1074.91 0 Z M 1084.61 0 L 285.092 840 L 283.635 840 L 1083.15 0 Z M 1092.85 0 L 293.335 840 L 291.878 840 L 1091.39 0 Z M 1101.09 0 L 301.577 840 L 300.12 840 L 1099.64 0 Z M 1109.33 0 L 309.819 840 L 308.362 840 L 1107.88 0 Z M 1117.58 0 L 318.062 840 L 316.605 840 L 1116.12 0 Z M 1125.82 0 L 326.304 840 L 324.847 840 L 1124.36 0 Z M 1134.06 0 L 334.547 840 L 333.09 840 L 1132.6 0 Z M 1142.3 0 L 342.789 840 L 341.332 840 L 1140.85 0 Z M 1150.55 0 L 351.031 840 L 349.575 840 L 1149.09 0 Z M 1158.79 0 L 359.274 840 L 357.817 840 L 1157.33 0 Z M 1167.03 0 L 367.516 840 L 366.059 840 L 1165.57 0 Z M 1175.27 0 L 375.759 840 L 374.302 840 L 1173.82 0 Z M 1183.52 0 L 384.001 840 L 382.544 840 L 1182.06 0 Z M 1191.76 0 L 392.244 840 L 390.787 840 L 1190.3 0 Z M 1200 0 L 400.486 840 L 399.029 840 L 1198.54 0 Z M 1208.24 0 L 408.728 840 L 407.272 840 L 1206.79 0 Z M 1216.49 0 L 416.971 840 L 415.514 840 L 1215.03 0 Z M 1224.73 0 L 425.213 840 L 423.756 840 L 1223.27 0 Z M 1232.97 0 L 433.456 840 L 431.999 840 L 1231.51 0 Z M 1241.21 0 L 441.698 840 L 440.241 840 L 1239.76 0 Z M 1249.46 0 L 449.941 840 L 448.484 840 L 1248 0 Z M 1257.7 0 L 458.183 840 L 456.726 840 L 1256.24 0 Z M 1265.94 0 L 466.425 840 L 464.969 840 L 1264.48 0 Z M 1274.18 0 L 474.668 840 L 473.211 840 L 1272.73 0 Z M 1282.43 0 L 482.91 840 L 481.453 840 L 1280.97 0 Z M 1290.67 0 L 491.153 840 L 489.696 840 L 1289.21 0 Z M 1298.91 0 L 499.395 840 L 497.938 840 L 1297.45 0 Z M 1307.15 0 L 507.638 840 L 506.181 840 L 1305.7 0 Z M 1315.4 0 L 515.88 840 L 514.423 840 L 1313.94 0 Z M 1323.64 0 L 524.122 840 L 522.665 840 L 1322.18 0 Z M 1331.88 0 L 532.365 840 L 530.908 840 L 1330.42 0 Z M 1340.12 0 L 540.607 840 L 539.15 840 L 1338.67 0 Z M 1348.36 0 L 548.85 840 L 547.393 840 L 1346.91 0 Z M 1356.61 0 L 557.092 840 L 555.635 840 L 1355.15 0 Z M 1360 5.095 L 565.335 840 L 563.878 840 L 1360 3.565 Z M 1360 13.755 L 573.577 840 L 572.12 840 L 1360 12.224 Z M 1360 22.415 L 581.819 840 L 580.362 840 L 1360 20.884 L 1360 22.414 Z M 1360 31.075 L 590.062 840 L 588.605 840 L 1360 29.544 L 1360 31.074 Z M 1360 39.735 L 598.304 840 L 596.847 840 L 1360 38.204 L 1360 39.734 Z M 1360 48.395 L 606.547 840 L 605.09 840 L 1360 46.864 L 1360 48.394 Z M 1360 57.054 L 614.789 840 L 613.332 840 L 1360 55.523 Z M 1360 65.714 L 623.031 840 L 621.575 840 L 1360 64.183 L 1360 65.713 Z M 1360 74.374 L 631.274 840 L 629.817 840 L 1360 72.843 L 1360 74.373 Z M 1360 83.034 L 639.516 840 L 638.059 840 L 1360 81.503 L 1360 83.033 Z M 1360 91.694 L 647.759 840 L 646.302 840 L 1360 90.163 L 1360 91.693 Z M 1360 100.353 L 656.001 840 L 654.544 840 L 1360 98.822 Z M 1360 109.013 L 664.244 840 L 662.787 840 L 1360 107.482 Z M 1360 117.673 L 672.486 840 L 671.029 840 L 1360 116.142 Z M 1360 126.332 L 680.728 840 L 679.272 840 L 1360 124.802 Z M 1360 134.992 L 688.971 840 L 687.514 840 L 1360 133.461 Z M 1360 143.652 L 697.213 840 L 695.756 840 L 1360 142.121 Z M 1360 152.312 L 705.456 840 L 703.999 840 L 1360 150.781 Z M 1360 160.972 L 713.698 840 L 712.241 840 L 1360 159.441 Z M 1360 169.631 L 721.941 840 L 720.484 840 L 1360 168.101 Z M 1360 178.291 L 730.183 840 L 728.726 840 L 1360 176.76 Z M 1360 186.951 L 738.425 840 L 736.968 840 L 1360 185.42 Z M 1360 195.611 L 746.668 840 L 745.211 840 L 1360 194.08 Z M 1360 204.27 L 754.91 840 L 753.453 840 L 1360 202.74 Z M 1360 212.93 L 763.153 840 L 761.696 840 L 1360 211.4 Z M 1360 221.59 L 771.395 840 L 769.938 840 L 1360 220.059 Z M 1360 230.25 L 779.638 840 L 778.181 840 L 1360 228.719 Z M 1360 238.91 L 787.88 840 L 786.423 840 L 1360 237.379 Z M 1360 247.569 L 796.122 840 L 794.665 840 L 1360 246.039 Z M 1360 256.229 L 804.365 840 L 802.908 840 L 1360 254.699 Z M 1360 264.889 L 812.607 840 L 811.15 840 L 1360 263.358 Z M 1360 273.549 L 820.85 840 L 819.393 840 L 1360 272.018 Z M 1360 282.209 L 829.092 840 L 827.635 840 L 1360 280.678 Z M 1360 290.868 L 837.334 840 L 835.878 840 L 1360 289.338 Z M 1360 299.528 L 845.577 840 L 844.12 840 L 1360 297.998 Z M 1360 308.188 L 853.819 840 L 852.362 840 L 1360 306.657 Z M 1360 316.848 L 862.062 840 L 860.605 840 L 1360 315.317 Z M 1360 325.508 L 870.304 840 L 868.847 840 L 1360 323.977 Z M 1360 334.167 L 878.547 840 L 877.09 840 L 1360 332.637 Z M 1360 342.827 L 886.789 840 L 885.332 840 L 1360 341.297 Z M 1360 351.487 L 895.031 840 L 893.575 840 L 1360 349.956 Z M 1360 360.147 L 903.274 840 L 901.817 840 L 1360 358.616 Z M 1360 368.807 L 911.516 840 L 910.059 840 L 1360 367.276 Z M 1360 377.466 L 919.759 840 L 918.302 840 L 1360 375.936 Z M 1360 386.126 L 928.001 840 L 926.544 840 L 1360 384.595 Z M 1360 394.786 L 936.244 840 L 934.787 840 L 1360 393.255 Z M 1360 403.446 L 944.486 840 L 943.029 840 L 1360 401.915 Z M 1360 412.106 L 952.728 840 L 951.272 840 L 1360 410.575 Z M 1360 420.765 L 960.971 840 L 959.514 840 L 1360 419.235 Z M 1360 429.425 L 969.213 840 L 967.756 840 L 1360 427.894 Z M 1360 438.085 L 977.456 840 L 975.999 840 L 1360 436.554 Z M 1360 446.745 L 985.698 840 L 984.241 840 L 1360 445.214 Z M 1360 455.405 L 993.941 840 L 992.484 840 L 1360 453.874 Z M 1360 464.064 L 1002.18 840 L 1000.73 840 L 1360 462.534 Z M 1360 472.724 L 1010.43 840 L 1008.97 840 L 1360 471.193 Z M 1360 481.384 L 1018.67 840 L 1017.21 840 L 1360 479.853 Z M 1360 490.044 L 1026.91 840 L 1025.45 840 L 1360 488.513 Z M 1360 498.703 L 1035.15 840 L 1033.7 840 L 1360 497.173 Z M 1360 507.363 L 1043.4 840 L 1041.94 840 L 1360 505.833 Z M 1360 516.023 L 1051.64 840 L 1050.18 840 L 1360 514.492 Z M 1360 524.683 L 1059.88 840 L 1058.42 840 L 1360 523.152 Z M 1360 533.343 L 1068.12 840 L 1066.67 840 L 1360 531.812 Z M 1360 542.002 L 1076.36 840 L 1074.91 840 L 1360 540.472 Z M 1360 550.662 L 1084.61 840 L 1083.15 840 L 1360 549.132 Z M 1360 559.322 L 1092.85 840 L 1091.39 840 L 1360 557.791 Z M 1360 567.982 L 1101.09 840 L 1099.64 840 L 1360 566.451 Z M 1360 576.642 L 1109.33 840 L 1107.88 840 L 1360 575.111 Z M 1360 585.301 L 1117.58 840 L 1116.12 840 L 1360 583.771 Z M 1360 593.961 L 1125.82 840 L 1124.36 840 L 1360 592.431 Z M 1360 602.621 L 1134.06 840 L 1132.6 840 L 1360 601.09 Z M 1360 611.281 L 1142.3 840 L 1140.85 840 L 1360 609.75 Z M 1360 619.941 L 1150.55 840 L 1149.09 840 L 1360 618.41 Z M 1360 628.6 L 1158.79 840 L 1157.33 840 L 1360 627.07 Z M 1360 637.26 L 1167.03 840 L 1165.57 840 L 1360 635.73 Z M 1360 645.92 L 1175.27 840 L 1173.82 840 L 1360 644.389 Z M 1360 654.58 L 1183.52 840 L 1182.06 840 L 1360 653.049 Z M 1360 663.24 L 1191.76 840 L 1190.3 840 L 1360 661.709 Z M 1360 671.899 L 1200 840 L 1198.54 840 L 1360 670.369 Z M 1360 680.559 L 1208.24 840 L 1206.79 840 L 1360 679.028 Z M 1360 689.219 L 1216.49 840 L 1215.03 840 L 1360 687.688 Z M 1360 697.879 L 1224.73 840 L 1223.27 840 L 1360 696.348 Z M 1360 706.539 L 1232.97 840 L 1231.51 840 L 1360 705.008 Z M 1360 715.198 L 1241.21 840 L 1239.76 840 L 1360 713.668 Z M 1360 723.858 L 1249.46 840 L 1248 840 L 1360 722.327 Z M 1360 732.518 L 1257.7 840 L 1256.24 840 L 1360 730.987 Z M 1360 741.178 L 1265.94 840 L 1264.48 840 L 1360 739.647 Z M 1360 749.838 L 1274.18 840 L 1272.73 840 L 1360 748.307 Z M 1360 758.497 L 1282.43 840 L 1280.97 840 L 1360 756.967 Z M 1360 767.157 L 1290.67 840 L 1289.21 840 L 1360 765.626 Z M 1360 775.817 L 1298.91 840 L 1297.45 840 L 1360 774.286 Z M 1360 784.477 L 1307.15 840 L 1305.7 840 L 1360 782.946 Z M 1360 793.136 L 1315.4 840 L 1313.94 840 L 1360 791.606 Z M 1360 801.796 L 1323.64 840 L 1322.18 840 L 1360 800.266 Z M 1360 810.456 L 1331.88 840 L 1330.42 840 L 1360 808.925 Z M 1360 819.116 L 1340.12 840 L 1338.67 840 L 1360 817.585 Z M 1360 827.776 L 1348.36 840 L 1346.91 840 L 1360 826.245 Z M 1360 836.435 L 1356.61 840 L 1355.15 840 L 1360 834.905 Z M 796.122 0 L 0 836.435 L 0 834.905 L 794.665 0 Z M 787.88 0 L 0 827.776 L 0 826.245 L 786.423 0 Z M 779.638 0 L 0 819.116 L 0 817.585 L 778.181 0 Z M 771.395 0 L 0 810.456 L 0 808.925 L 769.938 0 Z M 763.153 0 L 0 801.796 L 0 800.266 L 761.696 0 Z M 754.91 0 L 0 793.136 L 0 791.606 L 753.453 0 Z M 746.668 0 L 0 784.477 L 0 782.946 L 745.211 0 Z M 738.425 0 L 0 775.817 L 0 774.286 L 736.968 0 Z M 730.183 0 L 0 767.157 L 0 765.626 L 728.726 0 Z M 721.941 0 L 0 758.497 L 0 756.967 L 720.484 0 Z M 713.698 0 L 0 749.838 L 0 748.307 L 712.241 0 Z M 705.456 0 L 0 741.178 L 0 739.647 L 703.999 0 Z M 697.213 0 L 0 732.518 L 0 730.987 L 695.756 0 Z M 688.971 0 L 0 723.858 L 0 722.327 L 687.514 0 Z M 680.728 0 L 0 715.198 L 0 713.668 L 679.272 0 Z M 672.486 0 L 0 706.539 L 0 705.008 L 671.029 0 Z M 664.244 0 L 0 697.879 L 0 696.348 L 662.787 0 Z M 656.001 0 L 0 689.219 L 0 687.688 L 654.544 0 Z M 647.759 0 L 0 680.559 L 0 679.028 L 646.302 0 Z M 639.516 0 L 0 671.899 L 0 670.369 L 638.059 0 Z M 631.274 0 L 0 663.24 L 0 661.709 L 629.817 0 Z M 623.031 0 L 0 654.58 L 0 653.049 L 621.575 0 Z M 614.789 0 L 0 645.92 L 0 644.389 L 613.332 0 Z M 606.547 0 L 0 637.26 L 0 635.73 L 605.09 0 Z M 598.304 0 L 0 628.6 L 0 627.07 L 596.847 0 Z M 590.062 0 L 0 619.941 L 0 618.41 L 588.605 0 Z M 581.819 0 L 0 611.281 L 0 609.75 L 580.362 0 Z M 573.577 0 L 0 602.621 L 0 601.09 L 572.12 0 Z M 565.335 0 L 0 593.961 L 0 592.431 L 563.878 0 Z M 557.092 0 L 0 585.301 L 0 583.771 L 555.635 0 Z M 548.85 0 L 0 576.642 L 0 575.111 L 547.393 0 Z M 540.607 0 L 0 567.982 L 0 566.451 L 539.15 0 Z M 532.365 0 L 0 559.322 L 0 557.791 L 530.908 0 Z M 524.122 0 L 0 550.662 L 0 549.132 L 522.665 0 Z M 515.88 0 L 0 542.002 L 0 540.472 L 514.423 0 Z M 507.638 0 L 0 533.343 L 0 531.812 L 506.181 0 Z M 499.395 0 L 0 524.683 L 0 523.152 L 497.938 0 Z M 491.153 0 L 0 516.023 L 0 514.492 L 489.696 0 Z M 482.91 0 L 0 507.363 L 0 505.833 L 481.453 0 Z M 474.668 0 L 0 498.703 L 0 497.173 L 473.211 0 Z M 466.425 0 L 0 490.044 L 0 488.513 L 464.969 0 Z M 458.183 0 L 0 481.384 L 0 479.853 L 456.726 0 Z M 449.941 0 L 0 472.724 L 0 471.193 L 448.484 0 Z M 441.698 0 L 0 464.064 L 0 462.534 L 440.241 0 Z M 433.456 0 L 0 455.405 L 0 453.874 L 431.999 0 Z M 425.213 0 L 0 446.745 L 0 445.214 L 423.756 0 Z M 416.971 0 L 0 438.085 L 0 436.554 L 415.514 0 Z M 408.728 0 L 0 429.425 L 0 427.894 L 407.272 0 Z M 400.486 0 L 0 420.765 L 0 419.235 L 399.029 0 Z M 392.244 0 L 0 412.106 L 0 410.575 L 390.787 0 Z M 384.001 0 L 0 403.446 L 0 401.915 L 382.544 0 Z M 375.759 0 L 0 394.786 L 0 393.255 L 374.302 0 Z M 367.516 0 L 0 386.126 L 0 384.595 L 366.059 0 Z M 359.274 0 L 0 377.466 L 0 375.936 L 357.817 0 Z M 351.031 0 L 0 368.807 L 0 367.276 L 349.575 0 Z M 342.789 0 L 0 360.147 L 0 358.616 L 341.332 0 Z M 334.547 0 L 0 351.487 L 0 349.956 L 333.09 0 Z M 326.304 0 L 0 342.827 L 0 341.297 L 324.847 0 Z M 318.062 0 L 0 334.167 L 0 332.637 L 316.605 0 Z M 309.819 0 L 0 325.508 L 0 323.977 L 308.362 0 Z M 301.577 0 L 0 316.848 L 0 315.317 L 300.12 0 Z M 293.335 0 L 0 308.188 L 0 306.657 L 291.878 0 Z M 285.092 0 L 0 299.528 L 0 297.998 L 283.635 0 Z M 276.85 0 L 0 290.868 L 0 289.338 L 275.393 0 Z M 268.607 0 L 0 282.209 L 0 280.678 L 267.15 0 Z M 260.365 0 L 0 273.549 L 0 272.018 L 258.908 0 Z M 252.122 0 L 0 264.889 L 0 263.358 L 250.665 0 Z M 243.88 0 L 0 256.229 L 0 254.699 L 242.423 0 Z M 235.638 0 L 0 247.569 L 0 246.039 L 234.181 0 Z M 227.395 0 L 0 238.91 L 0 237.379 L 225.938 0 Z M 219.153 0 L 0 230.25 L 0 228.719 L 217.696 0 Z M 210.91 0 L 0 221.59 L 0 220.059 L 209.453 0 Z M 202.668 0 L 0 212.93 L 0 211.4 L 201.211 0 Z M 194.425 0 L 0 204.27 L 0 202.74 L 192.969 0 Z M 186.183 0 L 0 195.611 L 0 194.08 L 184.726 0 Z M 177.941 0 L 0 186.951 L 0 185.42 L 176.484 0 Z M 169.698 0 L 0 178.291 L 0 176.76 L 168.241 0 Z M 161.456 0 L 0 169.631 L 0 168.101 L 159.999 0 Z M 153.213 0 L 0 160.972 L 0 159.441 L 151.756 0 Z M 144.971 0 L 0 152.312 L 0 150.781 L 143.514 0 Z M 136.728 0 L 0 143.652 L 0 142.121 L 135.272 0 Z M 128.486 0 L 0 134.992 L 0 133.461 L 127.029 0 Z M 120.244 0 L 0 126.332 L 0 124.802 L 118.787 0 Z M 112.001 0 L 0 117.673 L 0 116.142 L 110.544 0 Z M 103.759 0 L 0 109.013 L 0 107.482 L 102.302 0 Z M 95.516 0 L 0 100.353 L 0 98.823 L 94.06 0 Z M 87.274 0 L 0 91.693 L 0 90.163 L 85.817 0 Z M 79.03 0 L 0 83.033 L 0 81.503 L 77.575 0 L 79.031 0 Z M 70.79 0 L 0 74.374 L 0 72.843 L 69.332 0 L 70.789 0 Z M 62.548 0 L 0 65.714 L 0 64.184 L 61.09 0 L 62.547 0 Z M 54.305 0 L 0 57.054 L 0 55.524 L 52.847 0 L 54.304 0 Z M 46.063 0 L 0 48.394 L 0 46.864 L 44.605 0 L 46.062 0 Z M 37.82 0 L 0 39.734 L 0 38.204 L 36.362 0 L 37.819 0 Z M 29.578 0 L 0 31.075 L 0 29.544 L 28.12 0 L 29.577 0 Z M 21.335 0 L 0 22.415 L 0 20.885 L 19.878 0 L 21.334 0 Z M 13.093 0 L 0 13.755 L 0 12.225 L 11.635 0 L 13.092 0 Z M 4.85 0 L 0 5.095 L 0 3.565 L 3.393 0 Z" fill="rgb(241, 242, 244)" height="840.0000302734375px" id="QpPT50KOU" width="1360.0000585937505px"/></g></svg>'
			}
			style={{
				...style,
			}}
		/>
	);
});
var css5 = [
	`.framer-QYSrD { -webkit-mask: ${mask}; aspect-ratio: 1.619047619047619; display: block; mask: ${mask}; user-select: none; width: 1360px; }`,
];
var Icon = withCSS(Component, css5, "framer-QYSrD");
Icon.displayName = "light";
var stdin_default = Icon;

// /:https://framerusercontent.com/modules/Ub0PpSy0bdM8z5hj01Cs/ueh3QIFNQzi6iDJ7TQ89/O9RB_C46j.js
import { jsx as _jsx2, jsxs as _jsxs } from "react/jsx-runtime";
import {
	addFonts,
	addPropertyControls,
	ControlType,
	cx as cx2,
	getFontsFromSharedStyle,
	Link,
	RichText,
	useComponentViewport,
	useLocaleInfo,
	useVariantState,
	withCSS as withCSS2,
} from "unframer";
import { LayoutGroup, motion as motion2, MotionConfigContext } from "unframer";
import * as React2 from "react";
import { useRef } from "react";

// /:https://framerusercontent.com/modules/b6UemWiXwO5364ADDnuu/TKRdxfzPsFuVbZZVEwTW/CCzJr7t2o.js
import { fontStore as fontStore5 } from "unframer";
fontStore5.loadFonts([
	"Inter-SemiBold",
	"Inter-Bold",
	"Inter-BoldItalic",
	"Inter-SemiBoldItalic",
]);
var fonts5 = [
	{
		explicitInter: true,
		fonts: [
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/hyOgCu0Xnghbimh0pE8QTvtt2AU.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/NeGmSOXrPBfEFIy5YZeHq17LEDA.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/oYaAX5himiTPYuN8vLWnqBbfD2s.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/lEJLP4R0yuCaMCjSXYHtJw72M.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/cRJyLNuTJR5jbyKzGi33wU9cqIQ.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/1ZFS7N918ojhhd0nQWdj3jz4w.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/A0Wcc7NgXMjUuFdquHDrIZpzZw0.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/DpPBYI0sL4fYLgAkX8KXOPVt7c.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/4RAEQdEOrcnDkhHiiCbJOw92Lk.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/1K3W8DizY3v4emK8Mb08YHxTbs.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/tUSCtfYVM1I1IchuyCwz9gDdQ.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/VgYFWiwsAC5OYxAycRXXvhze58.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/DXD0Q7LSl7HEvDzucnyLnGBHM.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "normal",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/GIryZETIX4IFypco5pYZONKhJIo.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/H89BbHkbHDzlxZzxi8uPzTsp90.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/u6gJwDuwB143kpNK1T1MDKDWkMc.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/43sJ6MfOPh1LCJt46OvyDuSbA6o.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/wccHG0r4gBDAIRhfHiOlq6oEkqw.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/WZ367JPwf9bRW6LdTHN8rXgSjw.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/QxmhnWTzLtyjIiZcfaLIJ8EFBXU.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/2A4Xx7CngadFGlVV4xrO06OBHY.woff2",
				weight: "700",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
				url: "https://framerusercontent.com/assets/vxBnBhH8768IFAXAb4Qf6wQHKs.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
				url: "https://framerusercontent.com/assets/zSsEuoJdh8mcFVk976C05ZfQr8.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+1F00-1FFF",
				url: "https://framerusercontent.com/assets/b8ezwLrN7h2AUoPEENcsTMVJ0.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange: "U+0370-03FF",
				url: "https://framerusercontent.com/assets/mvNEIBLyHbscgHtwfsByjXUz3XY.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
				url: "https://framerusercontent.com/assets/6FI2EneKzM3qBy5foOZXey7coCA.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
				url: "https://framerusercontent.com/assets/qrVgiXNd6RuQjETYQiVQ9nqCk.woff2",
				weight: "600",
			},
			{
				family: "Inter",
				source: "framer",
				style: "italic",
				unicodeRange:
					"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
				url: "https://framerusercontent.com/assets/NHHeAKJVP0ZWHk5YZnQQChIsBM.woff2",
				weight: "600",
			},
		],
	},
];
var css6 = [
	'.framer-fkUGr .framer-styles-preset-lz6hxz:not(.rich-text-wrapper), .framer-fkUGr .framer-styles-preset-lz6hxz.rich-text-wrapper p { --framer-font-family: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-bold-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-family-italic: "Inter", "Inter Placeholder", sans-serif; --framer-font-open-type-features: normal; --framer-font-size: 14px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-style-bold-italic: italic; --framer-font-style-italic: italic; --framer-font-variation-axes: normal; --framer-font-weight: 600; --framer-font-weight-bold: 700; --framer-font-weight-bold-italic: 700; --framer-font-weight-italic: 600; --framer-letter-spacing: 0em; --framer-line-height: 20px; --framer-paragraph-spacing: 20px; --framer-text-alignment: start; --framer-text-color: #ffffff; --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: none; }',
];
var className5 = "framer-fkUGr";

// /:https://framerusercontent.com/modules/Ub0PpSy0bdM8z5hj01Cs/ueh3QIFNQzi6iDJ7TQ89/O9RB_C46j.js
var enabledGestures = {
	Hqf5otrcw: {
		hover: true,
		pressed: true,
	},
	WbR6Pa8Dp: {
		hover: true,
		pressed: true,
	},
};
var cycleOrder = ["Hqf5otrcw", "WbR6Pa8Dp"];
var serializationHash = "framer-D8Gm7";
var variantClassNames = {
	Hqf5otrcw: "framer-v-17wg44f",
	WbR6Pa8Dp: "framer-v-1uorfbp",
};
function addPropertyOverrides(overrides, ...variants) {
	const nextOverrides = {};
	variants?.forEach(
		(variant) => variant && Object.assign(nextOverrides, overrides[variant]),
	);
	return nextOverrides;
}
var transition1 = {
	bounce: 0.2,
	delay: 0,
	duration: 0.4,
	type: "spring",
};
var Transition = ({ value, children }) => {
	const config = React2.useContext(MotionConfigContext);
	const transition = value ?? config.transition;
	const contextValue = React2.useMemo(
		() => ({
			...config,
			transition,
		}),
		[JSON.stringify(transition)],
	);
	return (
		<MotionConfigContext.Provider value={contextValue}>
			{children}
		</MotionConfigContext.Provider>
	);
};
var Variants = motion2.create(React2.Fragment);
var humanReadableVariantMap = {
	base: "WbR6Pa8Dp",
	sm: "Hqf5otrcw",
};
var getProps2 = ({
	height,
	id,
	label,
	link,
	showTrailingIcon,
	trailingIcon,
	width,
	...props
}) => {
	return {
		...props,
		fWelsHjNh: label ?? props.fWelsHjNh ?? "Start Free",
		FZzUU0mzQ: showTrailingIcon ?? props.FZzUU0mzQ ?? true,
		nDzoTHG6O: link ?? props.nDzoTHG6O,
		r6doNd5yc: trailingIcon ?? props.r6doNd5yc ?? "\uF061",
		variant:
			humanReadableVariantMap[props.variant] ?? props.variant ?? "Hqf5otrcw",
	};
};
var createLayoutDependency = (props, variants) => {
	if (props.layoutDependency)
		return variants.join("-") + props.layoutDependency;
	return variants.join("-");
};
var Component2 = /* @__PURE__ */ React2.forwardRef(function (props, ref) {
	const fallbackRef = useRef(null);
	const refBinding = ref ?? fallbackRef;
	const defaultLayoutId = React2.useId();
	const { activeLocale, setLocale } = useLocaleInfo();
	const componentViewport = useComponentViewport();
	const {
		style,
		className: className7,
		layoutId,
		variant,
		fWelsHjNh,
		FZzUU0mzQ,
		r6doNd5yc,
		nDzoTHG6O,
		...restProps
	} = getProps2(props);
	const {
		baseVariant,
		classNames,
		clearLoadingGesture,
		gestureHandlers,
		gestureVariant,
		isLoading,
		setGestureState,
		setVariant,
		variants,
	} = useVariantState({
		cycleOrder,
		defaultVariant: "Hqf5otrcw",
		enabledGestures,
		ref: refBinding,
		variant,
		variantClassNames,
	});
	const layoutDependency = createLayoutDependency(props, variants);
	const sharedStyleClassNames = [className5];
	const scopingClassNames = cx2(serializationHash, ...sharedStyleClassNames);
	return (
		<LayoutGroup id={layoutId ?? defaultLayoutId}>
			<Variants animate={variants} initial={false}>
				<Transition value={transition1}>
					<Link
						href={nDzoTHG6O}
						motionChild={true}
						nodeId={"Hqf5otrcw"}
						openInNewTab={false}
						scopeId={"O9RB_C46j"}
					>
						<motion2.a
							{...restProps}
							{...gestureHandlers}
							className={`${cx2(scopingClassNames, "framer-17wg44f", className7, classNames)} framer-1tq5591`}
							data-framer-name={"sm"}
							layoutDependency={layoutDependency}
							layoutId={"Hqf5otrcw"}
							ref={refBinding}
							style={{
								backgroundColor:
									"var(--token-0f4b74cd-af10-47b2-acab-dfe28d486776, rgb(26, 27, 29))",
								borderBottomLeftRadius: 10,
								borderBottomRightRadius: 10,
								borderTopLeftRadius: 10,
								borderTopRightRadius: 10,
								boxShadow:
									"0px 1px 1px 0px rgba(0, 0, 0, 0.05), 0px 0px 0px 1px rgba(255, 255, 255, 0.16)",
								...style,
							}}
							variants={{
								"Hqf5otrcw-hover": {
									backgroundColor:
										"var(--token-c97c57e2-811b-4eb3-bc26-8cb1e6f2f7cf, rgb(63, 63, 70))",
									boxShadow:
										"0px 1px 1px 0px rgba(0, 0, 0, 0.05), 0px 0px 0px 1px rgba(255, 255, 255, 0.24)",
								},
								"Hqf5otrcw-pressed": {
									backgroundColor:
										"var(--token-3f276bb8-7cda-4b6d-8836-ca7735f7598f, rgb(24, 24, 27))",
								},
								"WbR6Pa8Dp-hover": {
									backgroundColor:
										"var(--token-c97c57e2-811b-4eb3-bc26-8cb1e6f2f7cf, rgb(39, 39, 42))",
									boxShadow:
										"0px 1px 1px 0px rgba(0, 0, 0, 0.05), 0px 0px 0px 1px rgba(255, 255, 255, 0.24)",
								},
								"WbR6Pa8Dp-pressed": {
									backgroundColor:
										"var(--token-3f276bb8-7cda-4b6d-8836-ca7735f7598f, rgb(24, 24, 27))",
									boxShadow:
										"0px 1px 1px 0px rgba(0, 0, 0, 0.05), 0px 0px 0px 1px rgba(255, 255, 255, 0.24)",
								},
							}}
							{...addPropertyOverrides(
								{
									"Hqf5otrcw-hover": {
										"data-framer-name": void 0,
									},
									"Hqf5otrcw-pressed": {
										"data-framer-name": void 0,
									},
									"WbR6Pa8Dp-hover": {
										"data-framer-name": void 0,
									},
									"WbR6Pa8Dp-pressed": {
										"data-framer-name": void 0,
									},
									WbR6Pa8Dp: {
										"data-framer-name": "base",
									},
								},
								baseVariant,
								gestureVariant,
							)}
						>
							<RichText
								__fromCanvasComponent={true}
								className={"framer-m70m1e"}
								data-framer-name={"Start Free"}
								fonts={["Inter"]}
								layoutDependency={layoutDependency}
								layoutId={"J2y58ri6G"}
								style={{
									"--framer-paragraph-spacing": "0px",
									opacity: 1,
								}}
								text={fWelsHjNh}
								variants={{
									"Hqf5otrcw-pressed": {
										opacity: 0.5,
									},
									"WbR6Pa8Dp-hover": {
										opacity: 1,
									},
									"WbR6Pa8Dp-pressed": {
										opacity: 0.5,
									},
								}}
								verticalAlignment={"top"}
								withExternalLayout={true}
							>
								<React2.Fragment>
									<motion2.p
										className={"framer-styles-preset-lz6hxz"}
										data-styles-preset={"CCzJr7t2o"}
									>
										{"Start Free"}
									</motion2.p>
								</React2.Fragment>
							</RichText>
							{FZzUU0mzQ && (
								<RichText
									__fromCanvasComponent={true}
									className={"framer-bhemwi"}
									data-framer-name={"arrow-right"}
									fonts={["CUSTOM;Font Awesome 6 Pro Regular"]}
									layoutDependency={layoutDependency}
									layoutId={"q_ySf3o7E"}
									style={{
										"--extracted-r6o4lv":
											"var(--token-a68a9306-4dc4-48e6-8755-a34b33ff5aaa, rgb(255, 255, 255))",
										"--framer-paragraph-spacing": "0px",
										opacity: 1,
									}}
									text={r6doNd5yc}
									variants={{
										"Hqf5otrcw-pressed": {
											opacity: 0.5,
										},
										"WbR6Pa8Dp-hover": {
											opacity: 1,
										},
										"WbR6Pa8Dp-pressed": {
											opacity: 0.5,
										},
									}}
									verticalAlignment={"top"}
									withExternalLayout={true}
								>
									<React2.Fragment>
										<motion2.p
											style={{
												"--font-selector":
													"Q1VTVE9NO0ZvbnQgQXdlc29tZSA2IFBybyBSZWd1bGFy",
												"--framer-font-family":
													'"Font Awesome 6 Pro Regular", "Font Awesome 6 Pro Regular Placeholder", sans-serif',
												"--framer-font-size": "14px",
												"--framer-text-color":
													"var(--extracted-r6o4lv, var(--token-a68a9306-4dc4-48e6-8755-a34b33ff5aaa, rgb(255, 255, 255)))",
											}}
										>
											{"\uF061"}
										</motion2.p>
									</React2.Fragment>
								</RichText>
							)}
						</motion2.a>
					</Link>
				</Transition>
			</Variants>
		</LayoutGroup>
	);
});
var css7 = [
	"@supports (aspect-ratio: 1) { body { --framer-aspect-ratio-supported: auto; } }",
	".framer-D8Gm7.framer-1tq5591, .framer-D8Gm7 .framer-1tq5591 { display: block; }",
	".framer-D8Gm7.framer-17wg44f { align-content: center; align-items: center; cursor: pointer; display: flex; flex-direction: row; flex-wrap: nowrap; gap: 8px; height: 36px; justify-content: center; overflow: visible; padding: 10px 12px 10px 12px; position: relative; text-decoration: none; width: min-content; }",
	".framer-D8Gm7 .framer-m70m1e, .framer-D8Gm7 .framer-bhemwi { flex: none; height: auto; position: relative; white-space: pre; width: auto; }",
	".framer-D8Gm7.framer-v-1uorfbp.framer-17wg44f { height: 44px; padding: 10px 20px 10px 20px; }",
	...css6,
];
var FramerO9RB_C46j = withCSS2(Component2, css7, "framer-D8Gm7");
var stdin_default2 = FramerO9RB_C46j;
FramerO9RB_C46j.displayName = "ButtonPrimary";
FramerO9RB_C46j.defaultProps = {
	height: 36,
	width: 110,
};
addPropertyControls(FramerO9RB_C46j, {
	variant: {
		options: ["Hqf5otrcw", "WbR6Pa8Dp"],
		optionTitles: ["sm", "base"],
		title: "Variant",
		type: ControlType.Enum,
	},
	fWelsHjNh: {
		defaultValue: "Start Free",
		displayTextArea: false,
		title: "Label",
		type: ControlType.String,
	},
	FZzUU0mzQ: {
		defaultValue: true,
		title: "Show Trailing Icon",
		type: ControlType.Boolean,
	},
	r6doNd5yc: {
		defaultValue: "\uF061",
		displayTextArea: false,
		title: "Trailing Icon",
		type: ControlType.String,
	},
	nDzoTHG6O: {
		title: "Link",
		type: ControlType.Link,
	},
});
addFonts(
	FramerO9RB_C46j,
	[
		{
			explicitInter: true,
			fonts: [
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
					url: "https://framerusercontent.com/assets/5vvr9Vy74if2I6bQbJvbw7SY1pQ.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
					url: "https://framerusercontent.com/assets/EOr0mi4hNtlgWNn9if640EZzXCo.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+1F00-1FFF",
					url: "https://framerusercontent.com/assets/Y9k9QrlZAqio88Klkmbd8VoMQc.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+0370-03FF",
					url: "https://framerusercontent.com/assets/OYrD2tBIBPvoJXiIHnLoOXnY9M.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
					url: "https://framerusercontent.com/assets/JeYwfuaPfZHQhEG8U5gtPDZ7WQ.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
					url: "https://framerusercontent.com/assets/vQyevYAyHtARFwPqUzQGpnDs.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
					url: "https://framerusercontent.com/assets/b6Y37FthZeALduNqHicBT6FutY.woff2",
					weight: "400",
				},
				{
					family: "Font Awesome 6 Pro Regular",
					source: "custom",
					url: "https://framerusercontent.com/assets/8m1Zw0Lm7kiO8NiErxfHoP0.woff2",
				},
			],
		},
		...getFontsFromSharedStyle(fonts5),
	],
	{
		supportsExplicitInterCodegen: true,
	},
);

// /:https://framerusercontent.com/modules/ltWfmLj143UkesGgFZD7/i4FhgY0Snrz0OlddSKG3/qXRWmdo7f.js
import { jsx as _jsx3, jsxs as _jsxs2 } from "react/jsx-runtime";
import {
	addFonts as addFonts2,
	addPropertyControls as addPropertyControls2,
	ControlType as ControlType2,
	cx as cx3,
	getFontsFromSharedStyle as getFontsFromSharedStyle2,
	RichText as RichText2,
	useComponentViewport as useComponentViewport2,
	useLocaleInfo as useLocaleInfo2,
	useVariantState as useVariantState2,
	withCSS as withCSS3,
} from "unframer";
import {
	LayoutGroup as LayoutGroup2,
	motion as motion3,
	MotionConfigContext as MotionConfigContext2,
} from "unframer";
import * as React3 from "react";
import { useRef as useRef2 } from "react";

// /:https://framerusercontent.com/modules/0KhNcpjhv1h59aB1CXLL/zvowEZlImfADjhEjMZ0Y/xobAieqeY.js
import { fontStore as fontStore6 } from "unframer";
fontStore6.loadFonts(["GF;Geist Mono-500", "GF;Geist Mono-700"]);
var fonts6 = [
	{
		explicitInter: true,
		fonts: [
			{
				family: "Geist Mono",
				source: "google",
				style: "normal",
				url: "https://fonts.gstatic.com/s/geistmono/v3/or3yQ6H-1_WfwkMZI_qYPLs1a-t7PU0AbeEPKJ5W7ihaO_CS.woff2",
				weight: "500",
			},
			{
				family: "Geist Mono",
				source: "google",
				style: "normal",
				url: "https://fonts.gstatic.com/s/geistmono/v3/or3yQ6H-1_WfwkMZI_qYPLs1a-t7PU0AbeHaL55W7ihaO_CS.woff2",
				weight: "700",
			},
		],
	},
];
var css8 = [
	'.framer-gGjhH .framer-styles-preset-1t6j723:not(.rich-text-wrapper), .framer-gGjhH .framer-styles-preset-1t6j723.rich-text-wrapper p { --framer-font-family: "Geist Mono", monospace; --framer-font-family-bold: "Geist Mono", monospace; --framer-font-open-type-features: normal; --framer-font-size: 14px; --framer-font-style: normal; --framer-font-style-bold: normal; --framer-font-variation-axes: normal; --framer-font-weight: 500; --framer-font-weight-bold: 700; --framer-letter-spacing: 0em; --framer-line-height: 20px; --framer-paragraph-spacing: 20px; --framer-text-alignment: start; --framer-text-color: var(--token-d6940146-3092-44f2-b461-b1ae29af8487, #52525b); --framer-text-decoration: none; --framer-text-stroke-color: initial; --framer-text-stroke-width: initial; --framer-text-transform: uppercase; }',
];
var className6 = "framer-gGjhH";

// /:https://framerusercontent.com/modules/ltWfmLj143UkesGgFZD7/i4FhgY0Snrz0OlddSKG3/qXRWmdo7f.js
var serializationHash2 = "framer-2MFhQ";
var variantClassNames2 = {
	cxjza5XSy: "framer-v-wua8w4",
};
var transition12 = {
	bounce: 0.2,
	delay: 0,
	duration: 0.4,
	type: "spring",
};
var Transition2 = ({ value, children }) => {
	const config = React3.useContext(MotionConfigContext2);
	const transition = value ?? config.transition;
	const contextValue = React3.useMemo(
		() => ({
			...config,
			transition,
		}),
		[JSON.stringify(transition)],
	);
	return (
		<MotionConfigContext2.Provider value={contextValue}>
			{children}
		</MotionConfigContext2.Provider>
	);
};
var Variants2 = motion3.create(React3.Fragment);
var getProps3 = ({ height, icon, id, text, width, ...props }) => {
	return {
		...props,
		kWLkauSoU: icon ?? props.kWLkauSoU ?? "\uF1E6",
		QpJ3dK1Uw: text ?? props.QpJ3dK1Uw ?? "Placeholder",
	};
};
var createLayoutDependency2 = (props, variants) => {
	if (props.layoutDependency)
		return variants.join("-") + props.layoutDependency;
	return variants.join("-");
};
var Component3 = /* @__PURE__ */ React3.forwardRef(function (props, ref) {
	const fallbackRef = useRef2(null);
	const refBinding = ref ?? fallbackRef;
	const defaultLayoutId = React3.useId();
	const { activeLocale, setLocale } = useLocaleInfo2();
	const componentViewport = useComponentViewport2();
	const {
		style,
		className: className7,
		layoutId,
		variant,
		kWLkauSoU,
		QpJ3dK1Uw,
		...restProps
	} = getProps3(props);
	const {
		baseVariant,
		classNames,
		clearLoadingGesture,
		gestureHandlers,
		gestureVariant,
		isLoading,
		setGestureState,
		setVariant,
		variants,
	} = useVariantState2({
		defaultVariant: "cxjza5XSy",
		ref: refBinding,
		variant,
		variantClassNames: variantClassNames2,
	});
	const layoutDependency = createLayoutDependency2(props, variants);
	const sharedStyleClassNames = [className6];
	const scopingClassNames = cx3(serializationHash2, ...sharedStyleClassNames);
	return (
		<LayoutGroup2 id={layoutId ?? defaultLayoutId}>
			<Variants2 animate={variants} initial={false}>
				<Transition2 value={transition12}>
					<motion3.div
						{...restProps}
						{...gestureHandlers}
						className={cx3(
							scopingClassNames,
							"framer-wua8w4",
							className7,
							classNames,
						)}
						data-framer-name={"base"}
						layoutDependency={layoutDependency}
						layoutId={"cxjza5XSy"}
						ref={refBinding}
						style={{
							borderBottomLeftRadius: 10,
							borderBottomRightRadius: 10,
							borderTopLeftRadius: 10,
							borderTopRightRadius: 10,
							...style,
						}}
					>
						<RichText2
							__fromCanvasComponent={true}
							className={"framer-1jqn9k1"}
							data-framer-name={"plug"}
							fonts={["CUSTOM;Font Awesome 6 Pro Regular"]}
							layoutDependency={layoutDependency}
							layoutId={"zIqHkw7ko"}
							style={{
								"--extracted-r6o4lv":
									"var(--token-d6940146-3092-44f2-b461-b1ae29af8487, rgb(82, 82, 91))",
								"--framer-paragraph-spacing": "0px",
							}}
							text={kWLkauSoU}
							verticalAlignment={"top"}
							withExternalLayout={true}
						>
							<React3.Fragment>
								<motion3.p
									style={{
										"--font-selector":
											"Q1VTVE9NO0ZvbnQgQXdlc29tZSA2IFBybyBSZWd1bGFy",
										"--framer-font-family":
											'"Font Awesome 6 Pro Regular", "Font Awesome 6 Pro Regular Placeholder", sans-serif',
										"--framer-font-size": "15px",
										"--framer-text-color":
											"var(--extracted-r6o4lv, var(--token-d6940146-3092-44f2-b461-b1ae29af8487, rgb(82, 82, 91)))",
									}}
								>
									{"\uF1E6"}
								</motion3.p>
							</React3.Fragment>
						</RichText2>
						<RichText2
							__fromCanvasComponent={true}
							className={"framer-1mn6zyv"}
							data-framer-name={"API"}
							fonts={["Inter"]}
							layoutDependency={layoutDependency}
							layoutId={"MNsOHtmLI"}
							style={{
								"--framer-paragraph-spacing": "0px",
							}}
							text={QpJ3dK1Uw}
							verticalAlignment={"top"}
							withExternalLayout={true}
						>
							<React3.Fragment>
								<motion3.p
									className={"framer-styles-preset-1t6j723"}
									data-styles-preset={"xobAieqeY"}
								>
									{"Placeholder"}
								</motion3.p>
							</React3.Fragment>
						</RichText2>
					</motion3.div>
				</Transition2>
			</Variants2>
		</LayoutGroup2>
	);
});
var css9 = [
	"@supports (aspect-ratio: 1) { body { --framer-aspect-ratio-supported: auto; } }",
	".framer-2MFhQ.framer-jmd3me, .framer-2MFhQ .framer-jmd3me { display: block; }",
	".framer-2MFhQ.framer-wua8w4 { align-content: center; align-items: center; display: flex; flex-direction: row; flex-wrap: nowrap; gap: 6px; height: min-content; justify-content: center; overflow: visible; padding: 0px; position: relative; width: min-content; }",
	".framer-2MFhQ .framer-1jqn9k1, .framer-2MFhQ .framer-1mn6zyv { flex: none; height: auto; position: relative; white-space: pre; width: auto; }",
	...css8,
];
var FramerqXRWmdo7f = withCSS3(Component3, css9, "framer-2MFhQ");
var stdin_default3 = FramerqXRWmdo7f;
FramerqXRWmdo7f.displayName = "MonotypeOverline";
FramerqXRWmdo7f.defaultProps = {
	height: 20,
	width: 110,
};
addPropertyControls2(FramerqXRWmdo7f, {
	kWLkauSoU: {
		defaultValue: "\uF1E6",
		displayTextArea: false,
		title: "Icon",
		type: ControlType2.String,
	},
	QpJ3dK1Uw: {
		defaultValue: "Placeholder",
		description: "",
		displayTextArea: false,
		title: "Text",
		type: ControlType2.String,
	},
});
addFonts2(
	FramerqXRWmdo7f,
	[
		{
			explicitInter: true,
			fonts: [
				{
					family: "Font Awesome 6 Pro Regular",
					source: "custom",
					url: "https://framerusercontent.com/assets/8m1Zw0Lm7kiO8NiErxfHoP0.woff2",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
					url: "https://framerusercontent.com/assets/5vvr9Vy74if2I6bQbJvbw7SY1pQ.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
					url: "https://framerusercontent.com/assets/EOr0mi4hNtlgWNn9if640EZzXCo.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+1F00-1FFF",
					url: "https://framerusercontent.com/assets/Y9k9QrlZAqio88Klkmbd8VoMQc.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+0370-03FF",
					url: "https://framerusercontent.com/assets/OYrD2tBIBPvoJXiIHnLoOXnY9M.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
					url: "https://framerusercontent.com/assets/JeYwfuaPfZHQhEG8U5gtPDZ7WQ.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2074, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
					url: "https://framerusercontent.com/assets/vQyevYAyHtARFwPqUzQGpnDs.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
					url: "https://framerusercontent.com/assets/b6Y37FthZeALduNqHicBT6FutY.woff2",
					weight: "400",
				},
			],
		},
		...getFontsFromSharedStyle2(fonts6),
	],
	{
		supportsExplicitInterCodegen: true,
	},
);

// /:https://framer.com/m/for-developers-j22B.js
var LightFonts = getFonts(stdin_default);
var MonotypeOverlineFonts = getFonts(stdin_default3);
var ButtonPrimaryFonts = getFonts(stdin_default2);
var cycleOrder2 = ["R8tLJJzBh", "nEN9iYKa1", "cxmwk6zhN"];
var serializationHash3 = "framer-a2ZUD";
var variantClassNames3 = {
	cxmwk6zhN: "framer-v-1yqycon",
	nEN9iYKa1: "framer-v-sxrto8",
	R8tLJJzBh: "framer-v-m0k0kj",
};
function addPropertyOverrides2(overrides, ...variants) {
	const nextOverrides = {};
	variants?.forEach(
		(variant) => variant && Object.assign(nextOverrides, overrides[variant]),
	);
	return nextOverrides;
}
var transition13 = {
	bounce: 0.2,
	delay: 0,
	duration: 0.4,
	type: "spring",
};
var Transition3 = ({ value, children }) => {
	const config = React4.useContext(MotionConfigContext3);
	const transition = value ?? config.transition;
	const contextValue = React4.useMemo(
		() => ({
			...config,
			transition,
		}),
		[JSON.stringify(transition)],
	);
	return (
		<MotionConfigContext3.Provider value={contextValue}>
			{children}
		</MotionConfigContext3.Provider>
	);
};
var Variants3 = motion4.create(React4.Fragment);
var humanReadableVariantMap2 = {
	Desktop: "R8tLJJzBh",
	Phone: "cxmwk6zhN",
	Tablet: "nEN9iYKa1",
};
var getProps4 = ({ height, id, width, ...props }) => {
	return {
		...props,
		variant:
			humanReadableVariantMap2[props.variant] ?? props.variant ?? "R8tLJJzBh",
	};
};
var createLayoutDependency3 = (props, variants) => {
	if (props.layoutDependency)
		return variants.join("-") + props.layoutDependency;
	return variants.join("-");
};
var Component4 = /* @__PURE__ */ React4.forwardRef(function (props, ref) {
	const fallbackRef = useRef3(null);
	const refBinding = ref ?? fallbackRef;
	const defaultLayoutId = React4.useId();
	const { activeLocale, setLocale } = useLocaleInfo3();
	const componentViewport = useComponentViewport3();
	const {
		style,
		className: className7,
		layoutId,
		variant,
		...restProps
	} = getProps4(props);
	const {
		baseVariant,
		classNames,
		clearLoadingGesture,
		gestureHandlers,
		gestureVariant,
		isLoading,
		setGestureState,
		setVariant,
		variants,
	} = useVariantState3({
		cycleOrder: cycleOrder2,
		defaultVariant: "R8tLJJzBh",
		ref: refBinding,
		variant,
		variantClassNames: variantClassNames3,
	});
	const layoutDependency = createLayoutDependency3(props, variants);
	const sharedStyleClassNames = [className2, className, className3, className4];
	const scopingClassNames = cx4(serializationHash3, ...sharedStyleClassNames);
	return (
		<LayoutGroup3 id={layoutId ?? defaultLayoutId}>
			<Variants3 animate={variants} initial={false}>
				<Transition3 value={transition13}>
					<motion4.div
						{...restProps}
						{...gestureHandlers}
						className={cx4(
							scopingClassNames,
							"framer-m0k0kj",
							className7,
							classNames,
						)}
						data-framer-name={"Desktop"}
						layoutDependency={layoutDependency}
						layoutId={"R8tLJJzBh"}
						ref={refBinding}
						style={{
							...style,
						}}
						{...addPropertyOverrides2(
							{
								cxmwk6zhN: {
									"data-framer-name": "Phone",
								},
								nEN9iYKa1: {
									"data-framer-name": "Tablet",
								},
							},
							baseVariant,
							gestureVariant,
						)}
					>
						<motion4.div
							className={"framer-magxuu"}
							data-border={true}
							data-framer-name={"container"}
							layoutDependency={layoutDependency}
							layoutId={"moqhw_rV0"}
							style={{
								"--border-bottom-width": "0px",
								"--border-color":
									"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
								"--border-left-width": "1px",
								"--border-right-width": "1px",
								"--border-style": "solid",
								"--border-top-width": "0px",
							}}
						>
							<Link2
								href={"https://signatureapi.com/changelog"}
								motionChild={true}
								nodeId={"t1qEY4kDn"}
								scopeId={"dK6AuFGK_"}
							>
								<motion4.a
									className={"framer-jbs0jb framer-27wkyu"}
									data-border={true}
									data-framer-name={"pill"}
									layoutDependency={layoutDependency}
									layoutId={"t1qEY4kDn"}
									style={{
										"--border-bottom-width": "1px",
										"--border-color":
											"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
										"--border-left-width": "1px",
										"--border-right-width": "1px",
										"--border-style": "solid",
										"--border-top-width": "1px",
										backgroundColor:
											"var(--token-98ef648e-cc16-4e93-a787-b5e6b5bda39b, rgb(255, 255, 255))",
										borderBottomLeftRadius: 20,
										borderBottomRightRadius: 20,
										borderTopLeftRadius: 20,
										borderTopRightRadius: 20,
										boxShadow: "0px 1px 1px 0px rgba(0, 0, 0, 0.05)",
									}}
								>
									<RichText3
										__fromCanvasComponent={true}
										className={"framer-1mn2e9p"}
										data-framer-name={"Just launched Short Ceremony URLs"}
										fonts={["Inter"]}
										layoutDependency={layoutDependency}
										layoutId={"A8vHo8oX3"}
										style={{
											"--framer-paragraph-spacing": "0px",
										}}
										verticalAlignment={"top"}
										withExternalLayout={true}
									>
										<React4.Fragment>
											<motion4.p
												className={"framer-styles-preset-12cf9y3"}
												data-styles-preset={"GuhMGcyuK"}
											>
												{"Just launched Short Ceremony URLs"}
											</motion4.p>
										</React4.Fragment>
									</RichText3>
									<RichText3
										__fromCanvasComponent={true}
										className={"framer-z9cy7i"}
										data-framer-name={"angle-right"}
										fonts={["CUSTOM;Font Awesome 6 Pro Regular"]}
										layoutDependency={layoutDependency}
										layoutId={"ig0TSaXvJ"}
										style={{
											"--extracted-r6o4lv":
												"var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, rgb(24, 24, 27))",
											"--framer-paragraph-spacing": "0px",
										}}
										verticalAlignment={"top"}
										withExternalLayout={true}
									>
										<React4.Fragment>
											<motion4.p
												style={{
													"--font-selector":
														"Q1VTVE9NO0ZvbnQgQXdlc29tZSA2IFBybyBSZWd1bGFy",
													"--framer-font-family":
														'"Font Awesome 6 Pro Regular", sans-serif',
													"--framer-font-size": "14px",
													"--framer-text-color":
														"var(--extracted-r6o4lv, var(--token-21c8ed69-0c54-4c11-bfd2-6cc87f53f87a, rgb(24, 24, 27)))",
												}}
											>
												{"\uF105"}
											</motion4.p>
										</React4.Fragment>
									</RichText3>
								</motion4.a>
							</Link2>
							<RichText3
								__fromCanvasComponent={true}
								className={"framer-1iudqtl"}
								data-framer-name={"Electronic signatures for developers"}
								fonts={["Inter"]}
								layoutDependency={layoutDependency}
								layoutId={"VT4SMPcMX"}
								style={{
									"--framer-paragraph-spacing": "0px",
								}}
								verticalAlignment={"top"}
								withExternalLayout={true}
							>
								<React4.Fragment>
									<motion4.h1
										className={"framer-styles-preset-clpq28"}
										data-styles-preset={"BFWLS0wJE"}
									>
										{"Electronic signatures"}
									</motion4.h1>
									<motion4.h1
										className={"framer-styles-preset-clpq28"}
										data-styles-preset={"BFWLS0wJE"}
									>
										{"for developers"}
									</motion4.h1>
								</React4.Fragment>
							</RichText3>
							<RichText3
								__fromCanvasComponent={true}
								className={"framer-lmm9k2"}
								data-framer-name={
									"A flexible eSignature API that lets you build custom workflows, offer a focused signing experience, and integrate with thousands of apps effortlessly."
								}
								fonts={["Inter"]}
								layoutDependency={layoutDependency}
								layoutId={"LblcBjXZ9"}
								style={{
									"--framer-paragraph-spacing": "0px",
								}}
								verticalAlignment={"top"}
								withExternalLayout={true}
							>
								<React4.Fragment>
									<motion4.p
										className={"framer-styles-preset-e44ul3"}
										data-styles-preset={"olW1GQM24"}
									>
										{
											"A flexible eSignature API that lets you build custom workflows, offer a focused signing experience, and integrate with thousands of apps effortlessly."
										}
									</motion4.p>
								</React4.Fragment>
							</RichText3>
						</motion4.div>
						<motion4.div
							className={"framer-fw7rdy"}
							data-border={true}
							data-framer-name={"gtms"}
							layoutDependency={layoutDependency}
							layoutId={"E2nj3KmZL"}
							style={{
								"--border-bottom-width": "0px",
								"--border-color": "rgb(224, 227, 233)",
								"--border-left-width": "1px",
								"--border-right-width": "1px",
								"--border-style": "solid",
								"--border-top-width": "0px",
							}}
						>
							<motion4.div
								className={"framer-5qy669"}
								data-framer-name={"start-empty"}
								layoutDependency={layoutDependency}
								layoutId={"civEGOlwd"}
							>
								<motion4.div
									className={"framer-gek1h0"}
									data-framer-name={"l"}
									layoutDependency={layoutDependency}
									layoutId={"MlfTlpZMQ"}
								/>
								<motion4.div
									className={"framer-mzvje8"}
									data-border={true}
									data-framer-name={"c"}
									layoutDependency={layoutDependency}
									layoutId={"g46H97Uj4"}
									style={{
										"--border-bottom-width": "0px",
										"--border-color":
											"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
										"--border-left-width": "1px",
										"--border-right-width": "1px",
										"--border-style": "dashed",
										"--border-top-width": "0px",
									}}
								/>
								<motion4.div
									className={"framer-1rzmfu"}
									data-framer-name={"r"}
									layoutDependency={layoutDependency}
									layoutId={"xZNkkJC1Q"}
								/>
							</motion4.div>
							<motion4.div
								className={"framer-1et2g7q"}
								data-framer-name={"body"}
								layoutDependency={layoutDependency}
								layoutId={"iO62SZThW"}
							>
								<motion4.div
									className={"framer-jb8ltz"}
									data-border={true}
									data-framer-name={"l"}
									layoutDependency={layoutDependency}
									layoutId={"xZzwnUIkT"}
									style={{
										"--border-bottom-width": "1px",
										"--border-color":
											"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
										"--border-left-width": "0px",
										"--border-right-width": "0px",
										"--border-style": "dashed",
										"--border-top-width": "1px",
									}}
								>
									{_jsx4(stdin_default, {
										animated: true,
										className: "framer-1rnxx8g",
										layoutDependency,
										layoutId: "KATIzpYXv",
									})}
								</motion4.div>
								<motion4.div
									className={"framer-v047dp"}
									data-border={true}
									data-framer-name={"c"}
									layoutDependency={layoutDependency}
									layoutId={"jBJhAU8xZ"}
									style={{
										"--border-bottom-width": "1px",
										"--border-color": "rgb(224, 227, 233)",
										"--border-left-width": "1px",
										"--border-right-width": "1px",
										"--border-style": "solid",
										"--border-top-width": "1px",
									}}
								>
									<motion4.div
										className={"framer-176a55u"}
										data-border={true}
										data-framer-name={"gtm-block"}
										layoutDependency={layoutDependency}
										layoutId={"iwaBrw7pz"}
										style={{
											"--border-bottom-width": "0px",
											"--border-color":
												"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
											"--border-left-width": "0px",
											"--border-right-width": "1px",
											"--border-style": "solid",
											"--border-top-width": "1px",
										}}
										variants={{
											nEN9iYKa1: {
												"--border-right-width": "0px",
												"--border-top-width": "0px",
											},
										}}
									>
										<motion4.div
											className={"framer-3mg3yt"}
											data-framer-name={"top"}
											layoutDependency={layoutDependency}
											layoutId={"e6Z4ReEzA"}
										>
											<motion4.div
												className={"framer-1ubuxz8"}
												data-framer-name={"text"}
												layoutDependency={layoutDependency}
												layoutId={"Q3FkOJFHg"}
											>
												<ComponentViewportProvider
													height={20}
													y={
														(componentViewport?.y || 0) +
														0 +
														(((componentViewport?.height || 200) - 0 - 1e3) /
															2 +
															476 +
															0) +
														0 +
														24 +
														0 +
														0 +
														0 +
														0 +
														32 +
														0 +
														0 +
														0
													}
													{...addPropertyOverrides2(
														{
															cxmwk6zhN: {
																y:
																	(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1016) /
																		2 +
																		428 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	0 +
																	8 +
																	24 +
																	0 +
																	0 +
																	0,
															},
															nEN9iYKa1: {
																y:
																	(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1160) /
																		2 +
																		476 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	0 +
																	0 +
																	40 +
																	0 +
																	0 +
																	0,
															},
														},
														baseVariant,
														gestureVariant,
													)}
												>
													<SmartComponentScopedContainer
														className={"framer-yn2900-container"}
														layoutDependency={layoutDependency}
														layoutId={"QDavNUxTO-container"}
														nodeId={"QDavNUxTO"}
														rendersWithMotion={true}
														scopeId={"dK6AuFGK_"}
													>
														{_jsx4(stdin_default3, {
															height: "100%",
															id: "QDavNUxTO",
															kWLkauSoU: "\uF1E6",
															layoutId: "QDavNUxTO",
															QpJ3dK1Uw: "API",
															width: "100%",
														})}
													</SmartComponentScopedContainer>
												</ComponentViewportProvider>
												<RichText3
													__fromCanvasComponent={true}
													className={"framer-pqcs7"}
													data-framer-name={
														"Integrate electronic signatures into my app or platform"
													}
													fonts={["Inter"]}
													layoutDependency={layoutDependency}
													layoutId={"G1fpRcJyA"}
													style={{
														"--extracted-1wi65aa":
															"var(--token-04709e6b-ac96-47de-b348-7e8009cc46aa, rgb(161, 161, 170))",
														"--framer-paragraph-spacing": "0px",
													}}
													verticalAlignment={"top"}
													withExternalLayout={true}
												>
													<React4.Fragment>
														<motion4.h4
															className={"framer-styles-preset-1v2y8dz"}
															data-styles-preset={"vpUju7Kmv"}
														>
															{"Integrate electronic signatures"}
															<motion4.span
																style={{
																	"--framer-text-color":
																		"var(--extracted-1wi65aa, var(--token-04709e6b-ac96-47de-b348-7e8009cc46aa, rgb(161, 161, 170)))",
																}}
															>
																{" into my app or platform"}
															</motion4.span>
														</motion4.h4>
													</React4.Fragment>
												</RichText3>
											</motion4.div>
											<ComponentViewportProvider
												height={36}
												y={
													(componentViewport?.y || 0) +
													0 +
													(((componentViewport?.height || 200) - 0 - 1e3) / 2 +
														476 +
														0) +
													0 +
													24 +
													0 +
													0 +
													0 +
													0 +
													32 +
													96
												}
												{...addPropertyOverrides2(
													{
														cxmwk6zhN: {
															y:
																(componentViewport?.y || 0) +
																0 +
																(((componentViewport?.height || 200) -
																	0 -
																	1016) /
																	2 +
																	428 +
																	0) +
																0 +
																24 +
																0 +
																0 +
																0 +
																8 +
																24 +
																80,
														},
														nEN9iYKa1: {
															y:
																(componentViewport?.y || 0) +
																0 +
																(((componentViewport?.height || 200) -
																	0 -
																	1160) /
																	2 +
																	476 +
																	0) +
																0 +
																24 +
																0 +
																0 +
																0 +
																0 +
																40 +
																96,
														},
													},
													baseVariant,
													gestureVariant,
												)}
											>
												<SmartComponentScopedContainer
													className={"framer-19zkvu7-container"}
													layoutDependency={layoutDependency}
													layoutId={"UAZ17QLro-container"}
													nodeId={"UAZ17QLro"}
													rendersWithMotion={true}
													scopeId={"dK6AuFGK_"}
												>
													{_jsx4(stdin_default2, {
														fWelsHjNh: "API Docs",
														FZzUU0mzQ: true,
														height: "100%",
														id: "UAZ17QLro",
														layoutId: "UAZ17QLro",
														nDzoTHG6O:
															"https://signatureapi.com/docs/api/welcome",
														r6doNd5yc: "\uF061",
														style: {
															height: "100%",
														},
														variant: "Hqf5otrcw",
														width: "100%",
													})}
												</SmartComponentScopedContainer>
											</ComponentViewportProvider>
										</motion4.div>
										<Image
											background={{
												alt: "",
												fit: "fill",
												intrinsicHeight: 296,
												intrinsicWidth: 360,
												loading: getLoadingLazyAtYPosition(
													(componentViewport?.y || 0) +
														0 +
														(((componentViewport?.height || 200) - 0 - 1e3) /
															2 +
															476 +
															0) +
														0 +
														24 +
														0 +
														0 +
														0 +
														180,
												),
												pixelHeight: 592,
												pixelWidth: 720,
												sizes: `max(max(min(${componentViewport?.width || "100vw"} - 80px, 1360px) - 240px, 1px) / 3, 1px)`,
												src: "https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?scale-down-to=512&width=720&height=592",
												srcSet:
													"https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?width=720&height=592 720w",
											}}
											className={"framer-14nzjg4"}
											data-framer-name={"Img-1"}
											layoutDependency={layoutDependency}
											layoutId={"Y5xxoXg3j"}
											{...addPropertyOverrides2(
												{
													cxmwk6zhN: {
														background: {
															alt: "",
															fit: "fill",
															intrinsicHeight: 296,
															intrinsicWidth: 360,
															loading: getLoadingLazyAtYPosition(
																(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1016) /
																		2 +
																		428 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	0 +
																	0,
															),
															pixelHeight: 592,
															pixelWidth: 720,
															positionX: "left",
															positionY: "center",
															sizes: `calc(max(min(${componentViewport?.width || "100vw"} - 8px, 1360px) - 32px, 1px) * 0.4298)`,
															src: "https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?scale-down-to=512&width=720&height=592",
															srcSet:
																"https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?width=720&height=592 720w",
														},
													},
													nEN9iYKa1: {
														background: {
															alt: "",
															fit: "fill",
															intrinsicHeight: 296,
															intrinsicWidth: 360,
															loading: getLoadingLazyAtYPosition(
																(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1160) /
																		2 +
																		476 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	0 +
																	6,
															),
															pixelHeight: 592,
															pixelWidth: 720,
															sizes: "243.2432px",
															src: "https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?scale-down-to=512&width=720&height=592",
															srcSet:
																"https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/lf0YdEojTi97dIxC5Ala0ussAoQ.png?width=720&height=592 720w",
														},
													},
												},
												baseVariant,
												gestureVariant,
											)}
										/>
									</motion4.div>
									<motion4.div
										className={"framer-quy84"}
										data-border={true}
										data-framer-name={"gtm-block"}
										layoutDependency={layoutDependency}
										layoutId={"gULjxx0W1"}
										style={{
											"--border-bottom-width": "0px",
											"--border-color":
												"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
											"--border-left-width": "0px",
											"--border-right-width": "1px",
											"--border-style": "solid",
											"--border-top-width": "1px",
										}}
										variants={{
											cxmwk6zhN: {
												"--border-right-width": "0px",
											},
											nEN9iYKa1: {
												"--border-right-width": "0px",
											},
										}}
									>
										<motion4.div
											className={"framer-gn33v"}
											data-framer-name={"top"}
											layoutDependency={layoutDependency}
											layoutId={"Qm9kwdcoB"}
										>
											<motion4.div
												className={"framer-1pg6q4r"}
												data-framer-name={"text"}
												layoutDependency={layoutDependency}
												layoutId={"m78EYk_b8"}
											>
												<ComponentViewportProvider
													height={20}
													y={
														(componentViewport?.y || 0) +
														0 +
														(((componentViewport?.height || 200) - 0 - 1e3) /
															2 +
															476 +
															0) +
														0 +
														24 +
														0 +
														0 +
														0 +
														0 +
														32 +
														0 +
														0 +
														0
													}
													{...addPropertyOverrides2(
														{
															cxmwk6zhN: {
																y:
																	(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1016) /
																		2 +
																		428 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	180 +
																	0 +
																	24 +
																	0 +
																	0 +
																	0,
															},
															nEN9iYKa1: {
																y:
																	(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1160) /
																		2 +
																		476 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	212 +
																	0 +
																	40 +
																	0 +
																	0 +
																	0,
															},
														},
														baseVariant,
														gestureVariant,
													)}
												>
													<SmartComponentScopedContainer
														className={"framer-14gqqlt-container"}
														layoutDependency={layoutDependency}
														layoutId={"d2WOTkY1T-container"}
														nodeId={"d2WOTkY1T"}
														rendersWithMotion={true}
														scopeId={"dK6AuFGK_"}
													>
														{_jsx4(stdin_default3, {
															height: "100%",
															id: "d2WOTkY1T",
															kWLkauSoU: "\uF245",
															layoutId: "d2WOTkY1T",
															QpJ3dK1Uw: "no-code",
															width: "100%",
														})}
													</SmartComponentScopedContainer>
												</ComponentViewportProvider>
												<RichText3
													__fromCanvasComponent={true}
													className={"framer-1j9yb1b"}
													data-framer-name={
														"Integrate electronic signatures into my app or platform"
													}
													fonts={["Inter"]}
													layoutDependency={layoutDependency}
													layoutId={"P0NA_NkrO"}
													style={{
														"--extracted-1wi65aa":
															"var(--token-04709e6b-ac96-47de-b348-7e8009cc46aa, rgb(161, 161, 170))",
														"--framer-paragraph-spacing": "0px",
													}}
													verticalAlignment={"top"}
													withExternalLayout={true}
												>
													<React4.Fragment>
														<motion4.h4
															className={"framer-styles-preset-1v2y8dz"}
															data-styles-preset={"vpUju7Kmv"}
														>
															{"Automate sending and tracking"}
															<motion4.span
																style={{
																	"--framer-text-color":
																		"var(--extracted-1wi65aa, var(--token-04709e6b-ac96-47de-b348-7e8009cc46aa, rgb(161, 161, 170)))",
																}}
															>
																{" documents for signature"}
															</motion4.span>
														</motion4.h4>
													</React4.Fragment>
												</RichText3>
											</motion4.div>
											<ComponentViewportProvider
												height={36}
												y={
													(componentViewport?.y || 0) +
													0 +
													(((componentViewport?.height || 200) - 0 - 1e3) / 2 +
														476 +
														0) +
													0 +
													24 +
													0 +
													0 +
													0 +
													0 +
													32 +
													96
												}
												{...addPropertyOverrides2(
													{
														cxmwk6zhN: {
															y:
																(componentViewport?.y || 0) +
																0 +
																(((componentViewport?.height || 200) -
																	0 -
																	1016) /
																	2 +
																	428 +
																	0) +
																0 +
																24 +
																0 +
																0 +
																180 +
																0 +
																24 +
																96,
														},
														nEN9iYKa1: {
															y:
																(componentViewport?.y || 0) +
																0 +
																(((componentViewport?.height || 200) -
																	0 -
																	1160) /
																	2 +
																	476 +
																	0) +
																0 +
																24 +
																0 +
																0 +
																212 +
																0 +
																40 +
																96,
														},
													},
													baseVariant,
													gestureVariant,
												)}
											>
												<SmartComponentScopedContainer
													className={"framer-19jsgw4-container"}
													layoutDependency={layoutDependency}
													layoutId={"iyG7Bkafs-container"}
													nodeId={"iyG7Bkafs"}
													rendersWithMotion={true}
													scopeId={"dK6AuFGK_"}
												>
													{_jsx4(stdin_default2, {
														fWelsHjNh: "Power Automate",
														FZzUU0mzQ: true,
														height: "100%",
														id: "iyG7Bkafs",
														layoutId: "iyG7Bkafs",
														nDzoTHG6O:
															"https://signatureapi.com/docs/integrations/power-automate/getting-started",
														r6doNd5yc: "\uF061",
														style: {
															height: "100%",
														},
														variant: "Hqf5otrcw",
														width: "100%",
													})}
												</SmartComponentScopedContainer>
											</ComponentViewportProvider>
										</motion4.div>
										<Image
											background={{
												alt: "",
												fit: "fill",
												intrinsicHeight: 296,
												intrinsicWidth: 360,
												loading: getLoadingLazyAtYPosition(
													(componentViewport?.y || 0) +
														0 +
														(((componentViewport?.height || 200) - 0 - 1e3) /
															2 +
															476 +
															0) +
														0 +
														24 +
														0 +
														0 +
														0 +
														180,
												),
												pixelHeight: 592,
												pixelWidth: 720,
												sizes: `max(max(min(${componentViewport?.width || "100vw"} - 80px, 1360px) - 240px, 1px) / 3, 1px)`,
												src: "https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?scale-down-to=512&width=720&height=592",
												srcSet:
													"https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?width=720&height=592 720w",
											}}
											className={"framer-1muldzf"}
											data-framer-name={"Img-2"}
											layoutDependency={layoutDependency}
											layoutId={"joCxuqPRm"}
											{...addPropertyOverrides2(
												{
													cxmwk6zhN: {
														background: {
															alt: "",
															fit: "fill",
															intrinsicHeight: 296,
															intrinsicWidth: 360,
															loading: getLoadingLazyAtYPosition(
																(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1016) /
																		2 +
																		428 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	180 +
																	0,
															),
															pixelHeight: 592,
															pixelWidth: 720,
															positionX: "left",
															positionY: "center",
															sizes: `calc(max(min(${componentViewport?.width || "100vw"} - 8px, 1360px) - 32px, 1px) * 0.4649)`,
															src: "https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?scale-down-to=512&width=720&height=592",
															srcSet:
																"https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?width=720&height=592 720w",
														},
													},
													nEN9iYKa1: {
														background: {
															alt: "",
															fit: "fill",
															intrinsicHeight: 296,
															intrinsicWidth: 360,
															loading: getLoadingLazyAtYPosition(
																(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1160) /
																		2 +
																		476 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	212 +
																	6,
															),
															pixelHeight: 592,
															pixelWidth: 720,
															sizes: "243.2432px",
															src: "https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?scale-down-to=512&width=720&height=592",
															srcSet:
																"https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/zvJHp8WT5IURV6522VYOBhgWsO8.png?width=720&height=592 720w",
														},
													},
												},
												baseVariant,
												gestureVariant,
											)}
										/>
									</motion4.div>
									<motion4.div
										className={"framer-hei8i0"}
										data-framer-name={"gtm-block"}
										layoutDependency={layoutDependency}
										layoutId={"MYtUasBFz"}
										style={{
											"--border-bottom-width": "0px",
											"--border-color": "rgba(0, 0, 0, 0)",
											"--border-left-width": "0px",
											"--border-right-width": "0px",
											"--border-style": "solid",
											"--border-top-width": "0px",
										}}
										variants={{
											cxmwk6zhN: {
												"--border-bottom-width": "0px",
												"--border-color":
													"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
												"--border-left-width": "0px",
												"--border-right-width": "0px",
												"--border-style": "solid",
												"--border-top-width": "1px",
											},
											nEN9iYKa1: {
												"--border-bottom-width": "0px",
												"--border-color":
													"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
												"--border-left-width": "0px",
												"--border-right-width": "0px",
												"--border-style": "solid",
												"--border-top-width": "1px",
											},
										}}
										{...addPropertyOverrides2(
											{
												cxmwk6zhN: {
													"data-border": true,
												},
												nEN9iYKa1: {
													"data-border": true,
												},
											},
											baseVariant,
											gestureVariant,
										)}
									>
										<motion4.div
											className={"framer-n1ss5s"}
											data-framer-name={"top"}
											layoutDependency={layoutDependency}
											layoutId={"ZN88HzwnQ"}
										>
											<motion4.div
												className={"framer-3iamrh"}
												data-framer-name={"text"}
												layoutDependency={layoutDependency}
												layoutId={"kF94MktNP"}
											>
												<ComponentViewportProvider
													height={20}
													y={
														(componentViewport?.y || 0) +
														0 +
														(((componentViewport?.height || 200) - 0 - 1e3) /
															2 +
															476 +
															0) +
														0 +
														24 +
														0 +
														0 +
														0 +
														0 +
														32 +
														0 +
														0 +
														0
													}
													{...addPropertyOverrides2(
														{
															cxmwk6zhN: {
																y:
																	(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1016) /
																		2 +
																		428 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	360 +
																	0 +
																	24 +
																	0 +
																	0 +
																	0,
															},
															nEN9iYKa1: {
																y:
																	(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1160) /
																		2 +
																		476 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	424 +
																	0 +
																	40 +
																	0 +
																	0 +
																	0,
															},
														},
														baseVariant,
														gestureVariant,
													)}
												>
													<SmartComponentScopedContainer
														className={"framer-44lsxx-container"}
														layoutDependency={layoutDependency}
														layoutId={"zQb0Ftk9P-container"}
														nodeId={"zQb0Ftk9P"}
														rendersWithMotion={true}
														scopeId={"dK6AuFGK_"}
													>
														{_jsx4(stdin_default3, {
															height: "100%",
															id: "zQb0Ftk9P",
															kWLkauSoU: "\uF890",
															layoutId: "zQb0Ftk9P",
															QpJ3dK1Uw: "AI Toolkit",
															width: "100%",
														})}
													</SmartComponentScopedContainer>
												</ComponentViewportProvider>
												<RichText3
													__fromCanvasComponent={true}
													className={"framer-1eajpz6"}
													data-framer-name={
														"Integrate electronic signatures into my app or platform"
													}
													fonts={["Inter"]}
													layoutDependency={layoutDependency}
													layoutId={"SsHnkmcx6"}
													style={{
														"--extracted-1wi65aa":
															"var(--token-04709e6b-ac96-47de-b348-7e8009cc46aa, rgb(161, 161, 170))",
														"--framer-paragraph-spacing": "0px",
													}}
													verticalAlignment={"top"}
													withExternalLayout={true}
												>
													<React4.Fragment>
														<motion4.h4
															className={"framer-styles-preset-1v2y8dz"}
															data-styles-preset={"vpUju7Kmv"}
														>
															{"Enable own AI agent"}
															<motion4.span
																style={{
																	"--framer-text-color":
																		"var(--extracted-1wi65aa, var(--token-04709e6b-ac96-47de-b348-7e8009cc46aa, rgb(161, 161, 170)))",
																}}
															>
																{" to send and manage signed documents"}
															</motion4.span>
														</motion4.h4>
													</React4.Fragment>
												</RichText3>
											</motion4.div>
											<ComponentViewportProvider
												height={36}
												y={
													(componentViewport?.y || 0) +
													0 +
													(((componentViewport?.height || 200) - 0 - 1e3) / 2 +
														476 +
														0) +
													0 +
													24 +
													0 +
													0 +
													0 +
													0 +
													32 +
													96
												}
												{...addPropertyOverrides2(
													{
														cxmwk6zhN: {
															y:
																(componentViewport?.y || 0) +
																0 +
																(((componentViewport?.height || 200) -
																	0 -
																	1016) /
																	2 +
																	428 +
																	0) +
																0 +
																24 +
																0 +
																0 +
																360 +
																0 +
																24 +
																96,
														},
														nEN9iYKa1: {
															y:
																(componentViewport?.y || 0) +
																0 +
																(((componentViewport?.height || 200) -
																	0 -
																	1160) /
																	2 +
																	476 +
																	0) +
																0 +
																24 +
																0 +
																0 +
																424 +
																0 +
																40 +
																96,
														},
													},
													baseVariant,
													gestureVariant,
												)}
											>
												<SmartComponentScopedContainer
													className={"framer-1utr9wj-container"}
													layoutDependency={layoutDependency}
													layoutId={"Pndgz_GXA-container"}
													nodeId={"Pndgz_GXA"}
													rendersWithMotion={true}
													scopeId={"dK6AuFGK_"}
												>
													{_jsx4(stdin_default2, {
														fWelsHjNh: "AI Toolkit",
														FZzUU0mzQ: true,
														height: "100%",
														id: "Pndgz_GXA",
														layoutId: "Pndgz_GXA",
														nDzoTHG6O:
															"https://signatureapi.com/docs/ai-toolkit/agents",
														r6doNd5yc: "\uF061",
														style: {
															height: "100%",
														},
														variant: "Hqf5otrcw",
														width: "100%",
													})}
												</SmartComponentScopedContainer>
											</ComponentViewportProvider>
										</motion4.div>
										<Image
											background={{
												alt: "",
												fit: "fill",
												intrinsicHeight: 296,
												intrinsicWidth: 360,
												loading: getLoadingLazyAtYPosition(
													(componentViewport?.y || 0) +
														0 +
														(((componentViewport?.height || 200) - 0 - 1e3) /
															2 +
															476 +
															0) +
														0 +
														24 +
														0 +
														0 +
														0 +
														180,
												),
												pixelHeight: 592,
												pixelWidth: 720,
												sizes: `max(max(min(${componentViewport?.width || "100vw"} - 80px, 1360px) - 240px, 1px) / 3, 1px)`,
												src: "https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?scale-down-to=512&width=720&height=592",
												srcSet:
													"https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?width=720&height=592 720w",
											}}
											className={"framer-1e410uz"}
											data-framer-name={"Img-3"}
											layoutDependency={layoutDependency}
											layoutId={"ZDxLtXa2q"}
											{...addPropertyOverrides2(
												{
													cxmwk6zhN: {
														background: {
															alt: "",
															fit: "fill",
															intrinsicHeight: 296,
															intrinsicWidth: 360,
															loading: getLoadingLazyAtYPosition(
																(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1016) /
																		2 +
																		428 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	360 +
																	0,
															),
															pixelHeight: 592,
															pixelWidth: 720,
															positionX: "left",
															positionY: "center",
															sizes: `calc(max(min(${componentViewport?.width || "100vw"} - 8px, 1360px) - 32px, 1px) * 0.4649)`,
															src: "https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?scale-down-to=512&width=720&height=592",
															srcSet:
																"https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?width=720&height=592 720w",
														},
													},
													nEN9iYKa1: {
														background: {
															alt: "",
															fit: "fill",
															intrinsicHeight: 296,
															intrinsicWidth: 360,
															loading: getLoadingLazyAtYPosition(
																(componentViewport?.y || 0) +
																	0 +
																	(((componentViewport?.height || 200) -
																		0 -
																		1160) /
																		2 +
																		476 +
																		0) +
																	0 +
																	24 +
																	0 +
																	0 +
																	424 +
																	6,
															),
															pixelHeight: 592,
															pixelWidth: 720,
															sizes: "243.2432px",
															src: "https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?scale-down-to=512&width=720&height=592",
															srcSet:
																"https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?scale-down-to=512&width=720&height=592 512w,https://framerusercontent.com/images/TnyBxjppNw1CDUy90B0iec6c.png?width=720&height=592 720w",
														},
													},
												},
												baseVariant,
												gestureVariant,
											)}
										/>
									</motion4.div>
								</motion4.div>
								<motion4.div
									className={"framer-1i24xjt"}
									data-border={true}
									data-framer-name={"r"}
									layoutDependency={layoutDependency}
									layoutId={"z7WMRJkRI"}
									style={{
										"--border-bottom-width": "1px",
										"--border-color":
											"var(--token-bbd017ff-ff6f-4c0d-8247-5a16b6f347a5, rgb(224, 227, 233))",
										"--border-left-width": "0px",
										"--border-right-width": "0px",
										"--border-style": "dashed",
										"--border-top-width": "1px",
									}}
								>
									{_jsx4(stdin_default, {
										animated: true,
										className: "framer-stfp59",
										layoutDependency,
										layoutId: "WPCw0oJZW",
									})}
								</motion4.div>
							</motion4.div>
							<motion4.div
								className={"framer-5hftkr"}
								data-framer-name={"end-empty"}
								layoutDependency={layoutDependency}
								layoutId={"GKQhVkYwe"}
							>
								<motion4.div
									className={"framer-1yzpzzz"}
									data-framer-name={"l"}
									layoutDependency={layoutDependency}
									layoutId={"bVARe82p9"}
								/>
								<motion4.div
									className={"framer-1w79hzc"}
									data-border={true}
									data-framer-name={"c"}
									layoutDependency={layoutDependency}
									layoutId={"beUtY9sKW"}
									style={{
										"--border-bottom-width": "0px",
										"--border-color": "rgb(224, 227, 233)",
										"--border-left-width": "1px",
										"--border-right-width": "1px",
										"--border-style": "dashed",
										"--border-top-width": "0px",
									}}
								/>
								<motion4.div
									className={"framer-1jzcj2r"}
									data-framer-name={"r"}
									layoutDependency={layoutDependency}
									layoutId={"okWRq_O0a"}
								/>
							</motion4.div>
							<motion4.div
								className={"framer-jgu11c"}
								data-framer-name={"tl"}
								layoutDependency={layoutDependency}
								layoutId={"ekEj5lXBv"}
							>
								<motion4.div
									className={"framer-1cdka4a"}
									data-framer-name={"Rectangle 3"}
									layoutDependency={layoutDependency}
									layoutId={"Up0QI7DCi"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
									}}
								/>
								<motion4.div
									className={"framer-hxzkrw"}
									data-framer-name={"Rectangle 5"}
									layoutDependency={layoutDependency}
									layoutId={"vKmq5D216"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
										rotate: 90,
									}}
								/>
							</motion4.div>
							<motion4.div
								className={"framer-1fuaf1m"}
								data-framer-name={"tr"}
								layoutDependency={layoutDependency}
								layoutId={"ONcsOxjm5"}
							>
								<motion4.div
									className={"framer-1bsx4cc"}
									data-framer-name={"Rectangle 3"}
									layoutDependency={layoutDependency}
									layoutId={"UKmiaqNz_"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
									}}
								/>
								<motion4.div
									className={"framer-1uubanq"}
									data-framer-name={"Rectangle 5"}
									layoutDependency={layoutDependency}
									layoutId={"xXc0hVune"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
										rotate: 90,
									}}
								/>
							</motion4.div>
							<motion4.div
								className={"framer-1vem8c8"}
								data-framer-name={"bl"}
								layoutDependency={layoutDependency}
								layoutId={"TpJASHbjy"}
							>
								<motion4.div
									className={"framer-qt7xsx"}
									data-framer-name={"Rectangle 3"}
									layoutDependency={layoutDependency}
									layoutId={"zNQh1YW5n"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
									}}
								/>
								<motion4.div
									className={"framer-41gvrx"}
									data-framer-name={"Rectangle 5"}
									layoutDependency={layoutDependency}
									layoutId={"dXQmOb1Fw"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
										rotate: 90,
									}}
								/>
							</motion4.div>
							<motion4.div
								className={"framer-kbwqb"}
								data-framer-name={"br"}
								layoutDependency={layoutDependency}
								layoutId={"EvoI3A8eW"}
							>
								<motion4.div
									className={"framer-aihaip"}
									data-framer-name={"Rectangle 3"}
									layoutDependency={layoutDependency}
									layoutId={"UvS6AFvaS"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
									}}
								/>
								<motion4.div
									className={"framer-k0pvqw"}
									data-framer-name={"Rectangle 5"}
									layoutDependency={layoutDependency}
									layoutId={"hzUGizEFj"}
									style={{
										backgroundColor: "rgb(26, 27, 29)",
										rotate: 90,
									}}
								/>
							</motion4.div>
						</motion4.div>
					</motion4.div>
				</Transition3>
			</Variants3>
		</LayoutGroup3>
	);
});
var css10 = [
	"@supports (aspect-ratio: 1) { body { --framer-aspect-ratio-supported: auto; } }",
	".framer-a2ZUD.framer-27wkyu, .framer-a2ZUD .framer-27wkyu { display: block; }",
	".framer-a2ZUD.framer-m0k0kj { align-content: center; align-items: center; display: flex; flex-direction: column; flex-wrap: nowrap; gap: 0px; height: min-content; justify-content: center; overflow: visible; padding: 0px 40px 0px 40px; position: relative; width: 1200px; }",
	".framer-a2ZUD .framer-magxuu { align-content: center; align-items: center; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 20px; height: min-content; justify-content: center; max-width: 1360px; overflow: visible; padding: 80px 120px 80px 120px; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-jbs0jb { align-content: center; align-items: center; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; gap: 8px; height: 32px; justify-content: center; overflow: visible; padding: 10px 16px 10px 16px; position: relative; text-decoration: none; width: min-content; }",
	".framer-a2ZUD .framer-1mn2e9p, .framer-a2ZUD .framer-z9cy7i { flex: none; height: auto; position: relative; white-space: pre; width: auto; }",
	".framer-a2ZUD .framer-1iudqtl, .framer-a2ZUD .framer-pqcs7, .framer-a2ZUD .framer-1j9yb1b, .framer-a2ZUD .framer-1eajpz6 { flex: none; height: auto; position: relative; white-space: pre-wrap; width: 100%; word-break: break-word; word-wrap: break-word; }",
	".framer-a2ZUD .framer-lmm9k2 { flex: none; height: auto; max-width: 720px; position: relative; white-space: pre-wrap; width: 100%; word-break: break-word; word-wrap: break-word; }",
	".framer-a2ZUD .framer-fw7rdy { align-content: flex-start; align-items: flex-start; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 0px; height: min-content; justify-content: center; max-width: 1360px; overflow: hidden; padding: 0px; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-5qy669, .framer-a2ZUD .framer-5hftkr { align-content: flex-start; align-items: flex-start; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; gap: 0px; height: 24px; justify-content: flex-start; overflow: visible; padding: 0px; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-gek1h0, .framer-a2ZUD .framer-1rzmfu, .framer-a2ZUD .framer-1yzpzzz, .framer-a2ZUD .framer-1jzcj2r { align-content: flex-start; align-items: flex-start; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 10px; height: 100%; justify-content: flex-start; min-width: 120px; overflow: hidden; padding: 10px; position: relative; width: min-content; }",
	".framer-a2ZUD .framer-mzvje8, .framer-a2ZUD .framer-1w79hzc { align-content: flex-start; align-items: flex-start; display: flex; flex: 1 0 0px; flex-direction: column; flex-wrap: nowrap; gap: 10px; height: 100%; justify-content: flex-start; overflow: hidden; padding: 10px; position: relative; width: 1px; }",
	".framer-a2ZUD .framer-1et2g7q { align-content: flex-start; align-items: flex-start; display: flex; flex: none; flex-direction: row; flex-wrap: nowrap; gap: 0px; height: min-content; justify-content: flex-start; overflow: visible; padding: 0px; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-jb8ltz, .framer-a2ZUD .framer-1i24xjt { align-content: flex-start; align-items: flex-start; align-self: stretch; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 10px; height: auto; justify-content: flex-start; overflow: hidden; padding: 10px; position: relative; width: 120px; }",
	".framer-a2ZUD .framer-1rnxx8g, .framer-a2ZUD .framer-stfp59 { flex: none; height: var(--framer-aspect-ratio-supported, 816px); left: 0px; position: absolute; top: 0px; width: 1320px; z-index: 1; }",
	".framer-a2ZUD .framer-v047dp { align-content: flex-start; align-items: flex-start; display: flex; flex: 1 0 0px; flex-direction: row; flex-wrap: nowrap; gap: 0px; height: min-content; justify-content: flex-start; overflow: hidden; padding: 0px; position: relative; width: 1px; }",
	".framer-a2ZUD .framer-176a55u, .framer-a2ZUD .framer-quy84 { align-content: flex-start; align-items: flex-start; align-self: stretch; display: flex; flex: 1 0 0px; flex-direction: column; flex-wrap: nowrap; height: auto; justify-content: space-between; overflow: hidden; padding: 0px; position: relative; width: 1px; }",
	".framer-a2ZUD .framer-3mg3yt, .framer-a2ZUD .framer-gn33v, .framer-a2ZUD .framer-n1ss5s { align-content: flex-start; align-items: flex-start; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 32px; height: min-content; justify-content: flex-start; overflow: visible; padding: 32px 40px 16px 32px; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-1ubuxz8, .framer-a2ZUD .framer-1pg6q4r, .framer-a2ZUD .framer-3iamrh { align-content: flex-start; align-items: flex-start; display: flex; flex: none; flex-direction: column; flex-wrap: nowrap; gap: 16px; height: min-content; justify-content: flex-start; overflow: visible; padding: 0px; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-yn2900-container, .framer-a2ZUD .framer-14gqqlt-container, .framer-a2ZUD .framer-44lsxx-container { flex: none; height: auto; position: relative; width: auto; }",
	".framer-a2ZUD .framer-19zkvu7-container, .framer-a2ZUD .framer-19jsgw4-container, .framer-a2ZUD .framer-1utr9wj-container { flex: none; height: 36px; position: relative; width: auto; }",
	".framer-a2ZUD .framer-14nzjg4, .framer-a2ZUD .framer-1muldzf, .framer-a2ZUD .framer-1e410uz { aspect-ratio: 1.2162162162162162 / 1; flex: none; height: var(--framer-aspect-ratio-supported, 241px); overflow: visible; position: relative; width: 100%; }",
	".framer-a2ZUD .framer-hei8i0 { align-content: flex-start; align-items: flex-start; display: flex; flex: 1 0 0px; flex-direction: column; flex-wrap: nowrap; height: min-content; justify-content: space-between; overflow: hidden; padding: 0px; position: relative; width: 1px; }",
	".framer-a2ZUD .framer-jgu11c { flex: none; height: 15px; left: 113px; overflow: visible; position: absolute; top: 17px; width: 15px; z-index: 1; }",
	".framer-a2ZUD .framer-1cdka4a, .framer-a2ZUD .framer-hxzkrw, .framer-a2ZUD .framer-1bsx4cc, .framer-a2ZUD .framer-1uubanq, .framer-a2ZUD .framer-qt7xsx, .framer-a2ZUD .framer-41gvrx, .framer-a2ZUD .framer-aihaip, .framer-a2ZUD .framer-k0pvqw { flex: none; height: 15px; left: 7px; position: absolute; top: 0px; width: 1px; }",
	".framer-a2ZUD .framer-1fuaf1m { flex: none; height: 15px; overflow: visible; position: absolute; right: 113px; top: 17px; width: 15px; z-index: 1; }",
	".framer-a2ZUD .framer-1vem8c8 { bottom: 17px; flex: none; height: 15px; left: 113px; overflow: visible; position: absolute; width: 15px; z-index: 1; }",
	".framer-a2ZUD .framer-kbwqb { bottom: 17px; flex: none; height: 15px; overflow: visible; position: absolute; right: 113px; width: 15px; z-index: 1; }",
	".framer-a2ZUD.framer-v-sxrto8.framer-m0k0kj { padding: 0px 20px 0px 20px; width: 800px; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-gek1h0, .framer-a2ZUD.framer-v-sxrto8 .framer-1rzmfu, .framer-a2ZUD.framer-v-sxrto8 .framer-1yzpzzz, .framer-a2ZUD.framer-v-sxrto8 .framer-1jzcj2r { min-width: unset; width: 40px; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-jb8ltz, .framer-a2ZUD.framer-v-sxrto8 .framer-1i24xjt { width: 40px; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-1rnxx8g, .framer-a2ZUD.framer-v-sxrto8 .framer-stfp59, .framer-a2ZUD.framer-v-1yqycon .framer-1rnxx8g, .framer-a2ZUD.framer-v-1yqycon .framer-stfp59 { height: var(--framer-aspect-ratio-supported, 776px); }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-v047dp, .framer-a2ZUD.framer-v-1yqycon .framer-v047dp { flex-direction: column; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-176a55u, .framer-a2ZUD.framer-v-sxrto8 .framer-quy84 { align-content: center; align-items: center; align-self: unset; flex: none; flex-direction: row; height: min-content; width: 100%; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-3mg3yt, .framer-a2ZUD.framer-v-sxrto8 .framer-gn33v, .framer-a2ZUD.framer-v-sxrto8 .framer-n1ss5s { flex: 1 0 0px; padding: 40px; width: 1px; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-14nzjg4, .framer-a2ZUD.framer-v-sxrto8 .framer-1muldzf, .framer-a2ZUD.framer-v-sxrto8 .framer-1e410uz { height: var(--framer-aspect-ratio-supported, 200px); width: 243px; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-hei8i0 { align-content: center; align-items: center; flex: none; flex-direction: row; width: 100%; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-jgu11c, .framer-a2ZUD.framer-v-sxrto8 .framer-1vem8c8 { left: 33px; }",
	".framer-a2ZUD.framer-v-sxrto8 .framer-1fuaf1m, .framer-a2ZUD.framer-v-sxrto8 .framer-kbwqb { right: 33px; }",
	".framer-a2ZUD.framer-v-1yqycon.framer-m0k0kj { padding: 0px 4px 0px 4px; width: 390px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-magxuu { padding: 48px 24px 64px 24px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-gek1h0, .framer-a2ZUD.framer-v-1yqycon .framer-1rzmfu, .framer-a2ZUD.framer-v-1yqycon .framer-1yzpzzz, .framer-a2ZUD.framer-v-1yqycon .framer-1jzcj2r { min-width: unset; padding: 0px; width: 16px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-jb8ltz, .framer-a2ZUD.framer-v-1yqycon .framer-1i24xjt { padding: 0px; width: 16px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-176a55u, .framer-a2ZUD.framer-v-1yqycon .framer-quy84 { align-content: center; align-items: center; align-self: unset; flex: none; flex-direction: row; gap: 0px; height: min-content; justify-content: center; width: 100%; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-3mg3yt { flex: 1 0 0px; gap: 24px; padding: 24px 0px 24px 24px; width: 1px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-1ubuxz8 { gap: 8px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-14nzjg4 { aspect-ratio: unset; height: 180px; width: 43%; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-gn33v, .framer-a2ZUD.framer-v-1yqycon .framer-n1ss5s { flex: 1 0 0px; padding: 24px 0px 24px 24px; width: 1px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-1muldzf, .framer-a2ZUD.framer-v-1yqycon .framer-1e410uz { aspect-ratio: unset; height: 180px; width: 46%; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-hei8i0 { align-content: center; align-items: center; flex: none; flex-direction: row; gap: 0px; justify-content: center; width: 100%; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-jgu11c, .framer-a2ZUD.framer-v-1yqycon .framer-1vem8c8 { left: 9px; }",
	".framer-a2ZUD.framer-v-1yqycon .framer-1fuaf1m, .framer-a2ZUD.framer-v-1yqycon .framer-kbwqb { right: 9px; }",
	...css2,
	...css,
	...css3,
	...css4,
	'.framer-a2ZUD[data-border="true"]::after, .framer-a2ZUD [data-border="true"]::after { content: ""; border-width: var(--border-top-width, 0) var(--border-right-width, 0) var(--border-bottom-width, 0) var(--border-left-width, 0); border-color: var(--border-color, none); border-style: var(--border-style, none); width: 100%; height: 100%; position: absolute; box-sizing: border-box; left: 0; top: 0; border-radius: inherit; pointer-events: none; }',
];
var FramerdK6AuFGK_ = withCSS4(Component4, css10, "framer-a2ZUD");
var stdin_default4 = FramerdK6AuFGK_;
FramerdK6AuFGK_.displayName = "for-developers";
FramerdK6AuFGK_.defaultProps = {
	height: 917,
	width: 1200,
};
addPropertyControls3(FramerdK6AuFGK_, {
	variant: {
		options: ["R8tLJJzBh", "nEN9iYKa1", "cxmwk6zhN"],
		optionTitles: ["Desktop", "Tablet", "Phone"],
		title: "Variant",
		type: ControlType3.Enum,
	},
});
addFonts3(
	FramerdK6AuFGK_,
	[
		{
			explicitInter: true,
			fonts: [
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0460-052F, U+1C80-1C88, U+20B4, U+2DE0-2DFF, U+A640-A69F, U+FE2E-FE2F",
					url: "https://framerusercontent.com/assets/5vvr9Vy74if2I6bQbJvbw7SY1pQ.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+0301, U+0400-045F, U+0490-0491, U+04B0-04B1, U+2116",
					url: "https://framerusercontent.com/assets/EOr0mi4hNtlgWNn9if640EZzXCo.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+1F00-1FFF",
					url: "https://framerusercontent.com/assets/Y9k9QrlZAqio88Klkmbd8VoMQc.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange: "U+0370-03FF",
					url: "https://framerusercontent.com/assets/OYrD2tBIBPvoJXiIHnLoOXnY9M.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0100-024F, U+0259, U+1E00-1EFF, U+2020, U+20A0-20AB, U+20AD-20CF, U+2113, U+2C60-2C7F, U+A720-A7FF",
					url: "https://framerusercontent.com/assets/JeYwfuaPfZHQhEG8U5gtPDZ7WQ.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0000-00FF, U+0131, U+0152-0153, U+02BB-02BC, U+02C6, U+02DA, U+02DC, U+2000-206F, U+2070, U+2074-207E, U+2080-208E, U+20AC, U+2122, U+2191, U+2193, U+2212, U+2215, U+FEFF, U+FFFD",
					url: "https://framerusercontent.com/assets/GrgcKwrN6d3Uz8EwcLHZxwEfC4.woff2",
					weight: "400",
				},
				{
					family: "Inter",
					source: "framer",
					style: "normal",
					unicodeRange:
						"U+0102-0103, U+0110-0111, U+0128-0129, U+0168-0169, U+01A0-01A1, U+01AF-01B0, U+1EA0-1EF9, U+20AB",
					url: "https://framerusercontent.com/assets/b6Y37FthZeALduNqHicBT6FutY.woff2",
					weight: "400",
				},
				{
					family: "Font Awesome 6 Pro Regular",
					source: "custom",
					url: "https://framerusercontent.com/assets/8m1Zw0Lm7kiO8NiErxfHoP0.woff2",
				},
			],
		},
		...LightFonts,
		...MonotypeOverlineFonts,
		...ButtonPrimaryFonts,
		...getFontsFromSharedStyle3(fonts2),
		...getFontsFromSharedStyle3(fonts),
		...getFontsFromSharedStyle3(fonts3),
		...getFontsFromSharedStyle3(fonts4),
	],
	{
		supportsExplicitInterCodegen: true,
	},
);

// virtual:for-developers
import { WithFramerBreakpoints } from "unframer";

// virtual:__routes
var routes = {
	DhvwGaaou: {
		path: "/home-backup",
	},
	DiKMHAohF: {
		path: "/blog/:slug",
	},
	I4RMCmkQd: {
		path: "/e-signature-news",
	},
	KSjCJwy_s: {
		path: "/blog",
	},
	NukTy_VHU: {
		path: "/home-2",
	},
	OL4NtcSBu: {
		path: "/pricing",
	},
	QqS4d__RJ: {
		path: "/changelog",
	},
	augiA20Il: {
		path: "/",
	},
	elJhlphhA: {
		path: "/pricing-3",
	},
	kw24qmVnO: {
		path: "/pricing-2",
	},
	pCsjntyo1: {
		path: "/e-signature-news-2",
	},
	wKxO4rmxX: {
		path: "/home-for-export",
	},
};

// virtual:for-developers
import { jsx } from "react/jsx-runtime";
var locales = [];
var defaultResponsiveVariants = {
	base: "cxmwk6zhN",
	md: "nEN9iYKa1",
	xl: "R8tLJJzBh",
};
/** @type {function(Props): any} */
function ComponentWithRoot({ locale, ...rest }) {
	return (
		<ContextProviders
			routes={routes}
			framerSiteId={
				"ca611bf9ae534859de674b0501c1ab4e651b1f0645f4e150d7cde31ac347548b"
			}
			locale={locale}
			locales={locales}
		>
			{jsx(stdin_default4, {
				...rest,
			})}
		</ContextProviders>
	);
}
/**
 * @type {import("unframer").UnframerBreakpoint}
 * Represents a responsive breakpoint for unframer.
 */

/**
 * @typedef VariantsMap
 * Partial record of UnframerBreakpoint to Props.variant, with a mandatory 'base' key.
 * { [key in UnframerBreakpoint]?: Props['variant'] } & { base: Props['variant'] }
 */

/**
 * Renders ForDevelopersFramerComponent for all breakpoints with a variants map. Variant prop is inferred per breakpoint.
 * @function
 * @param {Omit<Props, 'variant'> & {variants?: VariantsMap}} props
 * @returns {any}
 */
ComponentWithRoot.Responsive = ({ locale = "", ...rest }) => {
	return (
		<ContextProviders
			routes={routes}
			framerSiteId={
				"ca611bf9ae534859de674b0501c1ab4e651b1f0645f4e150d7cde31ac347548b"
			}
			locale={locale}
			locales={locales}
		>
			<WithFramerBreakpoints
				Component={stdin_default4}
				variants={defaultResponsiveVariants}
				{...rest}
			/>
		</ContextProviders>
	);
};
Object.assign(ComponentWithRoot, stdin_default4);
var for_developers_default = ComponentWithRoot;
export { for_developers_default as default };
